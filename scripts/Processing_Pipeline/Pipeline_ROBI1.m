...
%subjects = '/home/data/subjects/MBG_subjects.txt';
%subjects = '/home/data/subjects/processing_list_060717.txt';
%subjects = '/home/data/subjects/list_forJDP.txt';

%subjects = {'Arsenio'};
subjects = {'ROBI001', 'ROBI002', 'ROBI004', 'ROBI005', 'ROBI006', 'ROBI007', 'ROBI008', 'ROBI009', 'ROBI010', 'ROBI012', 'ROBI013', 'ROBI014', 'ROBI015', 'ROBI017', 'ROBI018'};
%subjects = {'MBG001'    'MBG002'    'MBG004'    'MBG005'    'MBG006'    'MBG007'    'MBG008'    'MBG010'    'MBG011'    'MBG012'    'MBG013'    'MBG014'  'MBG015'    'MBG016'    'MBG017'    'MBG018'    'MBG020'    'MBG021'    'MBG022'    'MBG023'    'MBG024'};
%{'MAV029','MAV035'};
%
...
QCtrackingfile = '/home/data/subjects/QC/Allruns_toexclude.txt';
...
if isempty(QCtrackingfile)
    runs_toexclude = [];
else
    runs_toexclude = textread(QCtrackingfile,'%s');
end
...
if any([copy_rawBOLDdata T1_preprocess T1_segment T1_segment_fs6 T2_preprocess surface_registration DTI_preprocess BOLD_preprocess])
...
%for subnum = 1:subcount
    ...
    try
        ...
        freesurfer6folder = ['/home/data/subjects/' subject '/freesurfer6/']; 
        ...
        if copy_rawBOLDdata
            ...
            for seq = 1:length(functional_sequences)
                [~,~] = system(['rm ' preprocfolder '/' functional_sequences{seq} '_*']);
            end
        end
...
%                     end
                    ...
                    if correctsize && data_islong && (~any(strcmp(filename,runs_toexclude)))
        ...
        if T1_preprocess
            ...
            try delete([preprocfolder '/struct.anat/*']);
            rmdir([preprocfolder '/struct.anat'],'s'); catch; end
        ...
        if T1_segment
         ...
         %% T1 Segmentation freesurfer6
        
        if T1_segment_fs6
            
            disp(['Subject ' subject ': segmenting T1 with FreeSurfer 6']);
            dlmwrite(logfile,'segmenting T1 with freesurfer 6','-append','delimiter','')
            
            try [~,~,~] = rmdir(freesurfer6folder,'s'); catch; end
            
            mkdir(freesurfer6folder)
            
            %copy bias-corrected T1 to freesurfer folder
            copyfile([preprocfolder '/' T1file '.nii.gz'],[freesurfer6folder '/' T1file '.nii.gz'])
            cd(freesurfer6folder)
            
            %transform T1 to .mgz format
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['setenv FREESURFER_HOME /usr/local/freesurfer6/freesurfer;'...
                                                                    'source $FREESURFER_HOME/SetUpFreeSurfer.csh;'...
                                                                    'recon-all -subjid ' subject ' -sd ' freesurfer6folder ' -i ' freesurfer6folder '/' T1file '.nii.gz']);
            %segment T1
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['setenv FREESURFER_HOME /usr/local/freesurfer6/freesurfer;'...
                                                                    'source $FREESURFER_HOME/SetUpFreeSurfer.csh;'...
                                                                    'recon-all -subjid ' subject ' -sd ' freesurfer6folder ' -all']);
                                                                
            %move files around
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['mv ' freesurfer6folder '/' subject '/* ' freesurfer6folder]);
            rmdir([freesurfer6folder '/' subject ])
            cd(preprocfolder)
            
            if any(cell2mat(systemresult(:,1)))
                error(['Subject ' subject ': Problem detected with Freesurfer 6'])
            end
...
%             sessions = sessions(3:end);
            ...
            for sess = 1:length(sessions)
                ...
                for i = 1:length(DTIfiles_thissess)
                    ...
                    dlmwrite(sess_tracker,[DTIfile '_ec_FA_MNI.nii.gz ' num2str(sessnums(sess))],'-append','delimiter','');%write the data to the output file
            ...
            for filenum = 1:DTIcounter
                ...
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' DTIfile '_ec_b0 -ref ' preprocfolder '/' T1file '_bet -dof 6 -omat ' DTIfile '_2T1_init.mat']);
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' DTIfile '_ec_b0 -ref '  preprocfolder '/' T1file ' -dof 12 -cost bbr -wmseg ' preprocfolder '/aparc+aseg_cerebralwm -init ' DTIfile '_2T1_init.mat -omat ' DTIfile '_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
                ...
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' preprocfolder '/' T1file ' -interp spline -ref ' DTIfile '_ec_b0 -applyxfm -init T1_2' DTIfile '.mat -out ' T1file '_' DTIfile]);
                ...
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths ' T1file '_' DTIfile ' -thr 10 -bin ' T1file '_' DTIfile '_mask']);
                
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['dtifit -k ' DTIfile '_ec -m ' T1file '_' DTIfile '_mask -r ' DTIfile '.bvec -b ' DTIfile '.bval -o ' DTIfile '_ec']);
            ...
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' preprocfolder '/' T1file '_bet_MNI -ref ' preprocfolder '/' T1file '_bet_MNI -applyisoxfm ' num2str(DTI_normalized_voxdim) ' -init /usr/local/fsl/etc/flirtsch/ident.mat -o ' T1file '_bet_MNI_' num2str(DTI_normalized_voxdim) num2str(DTI_normalized_voxdim) num2str(DTI_normalized_voxdim)]);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths ' T1file '_bet_MNI_' num2str(DTI_normalized_voxdim) num2str(DTI_normalized_voxdim) num2str(DTI_normalized_voxdim) ' -thr 10 -bin ' T1file '_bet_MNI_' num2str(DTI_normalized_voxdim) num2str(DTI_normalized_voxdim) num2str(DTI_normalized_voxdim) '_mask'])
...
%                 end
                ...
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' bestrun_name{seq} '_st_mcf_avg_bet' fmstring ' -ref '  T1file ' -dof 6 -omat ' bestrun_name{seq} '_avg_2T1_init.mat']);
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' bestrun_name{seq} '_st_mcf_avg_bet' fmstring ' -ref '  T1file ' -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' bestrun_name{seq} '_avg_2T1_init.mat -omat ' bestrun_name{seq} '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
...
if any([T1_hippocampal_subfields BOLD_fcprocess cifti_creation cifti_correlation])
...
for subnum = 1:subcount
    ...
    try
        ...
        if T1_hippocampal_subfields
            ...
            %move files around
            [~,~] = system(['rm /home/data/subjects/' subject '/freesurfer6/scripts/IsRunning.lh+rh'])
            mkdir([freesurfer6folder '/../temp/']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['mv ' freesurfer6folder '/* ' freesurfer6folder '/../temp/']);
            mkdir([freesurfer6folder '/' subject]);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['mv ' freesurfer6folder '/../temp/* ' freesurfer6folder '/' subject '/']);
            rmdir([freesurfer6folder '/../temp/'])
            
            %segment hippocampal subfields from T1
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['setenv FREESURFER_HOME /usr/local/freesurfer6/freesurfer;'...
                                                                    'source $FREESURFER_HOME/SetUpFreeSurfer.csh;'...
                                                                    'recon-all -subjid ' subject ' -sd ' freesurfer6folder ' -hippocampal-subfields-T1']);
            
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['mri_convert -rl ' freesurfer6folder '/' subject '/mri/rawavg.mgz -rt nearest ' freesurfer6folder '/' subject '/mri/lh.hippoSfLabels-T1.v10.FSvoxelSpace.mgz ' freesurfer6folder '/' subject '/mri/lh.hippoSfLabels-T1.v10.T1voxelSpace.nii.gz']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['mri_convert -rl ' freesurfer6folder '/' subject '/mri/rawavg.mgz -rt nearest ' freesurfer6folder '/' subject '/mri/rh.hippoSfLabels-T1.v10.FSvoxelSpace.mgz ' freesurfer6folder '/' subject '/mri/rh.hippoSfLabels-T1.v10.T1voxelSpace.nii.gz']);
        ...
        if BOLD_fcprocess
            ...
            for seq = 1:length(functional_sequences)
                if fcprocess_sequences{seq}
                    ...
                    %FC_Process_filt_Avi(subject,['/home/data/subjects/' subject '/preprocessed/BOLDruns_sessions_' functional_sequences{seq} '.txt'],FDthresh,functional_sequence_TRs{seq},['/home/data/subjects/' subject '/fc_processed/'],functional_sequences{seq})
                    FC_Process_filt_Avi_norunsessregression(subject,['/home/data/subjects/' subject '/preprocessed/BOLDruns_sessions_' functional_sequences{seq} '.txt'],FDthresh,functional_sequence_TRs{seq},['/home/data/subjects/' subject '/fc_processed/'],functional_sequences{seq})
