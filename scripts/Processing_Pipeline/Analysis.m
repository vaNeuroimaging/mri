...
subjects = '/home/data/subjects/processing_list_073117.txt';
% subjects = '/home/data/subjects/processing_list_062017.txt';
%subjects = '/home/data/Analysis/MAV_subtyping/MAV_subjectlist.txt';
%subjects = '/home/data/subjects/MAV_final_list.txt';
%subjects = {'MAV070','MAV072'};
...
groupparcel_correlations = 0;
template_matching = 0;
infomap = 1;
parcellation = 0;
homogeneity_testing = 0;
parcel_infomap = 0;
groupparcel_infomap = 0;
...
infomapthresholds = [.003 .004 [.005 : .005 : .05]];% [.06 : .01 : .1] [.125 :.025 .2]];
infomap_minsize = 400;
infomapreps = 50;
            
parcel_infomapthresholds = [.005 : .005: .2];
...
templatesfile = '/home/data/scripts/Template_Matching/Templates_consensus_17.mat';
templates_kden = .05;
    ...
    %try
        ...
        if groupparcel_correlations
            ...
            for seq = 1:length(functional_sequences)
                ...
                corrmat = data.data;
                save([correlationfolder functional_sequences{seq} '_' parcellation_shortname '_corr.mat'],'corrmat')
        ...
        if infomap
            ...
            for seq = 1:length(functional_sequences)
                ...
                if ~exist(corrfile)
                    datafile = [ciftifolder '/cifti_timeseries_normalwall/' functional_sequences{seq} '_LR_surf_subcort_333_32k_fsLR_smooth2.55.dtseries.nii'];
                    data = ft_read_cifti_mod(datafile);
                    corrfile = paircorr_mod(data.data');
                    clear data
                    corrfile(isnan(corrfile)) = 0;
                end
                
                %dmatfile = [ciftifolder '/distances/normalwall_distmat.dconn.nii'];
                %if ~exist(dmatfile)
                    dmatfile = '/home/data/scripts/Resources/Conte69_atlas-v2.LR.32k_fs_LR.wb/Conte69.LR.32k_fs_LR.distances_surfgeo_voleuc_normalwall_xhemlarge_standardsubcort_uint8.mat';
                %end
                
                Run_Infomap(corrfile, dmatfile, xdist, infomapthresholds, 0, outfolder,infomapreps);
                clear corrfile
                ...
                cifti_to_border_v2([outfolder '/rawassn_minsize' num2str(infomap_minsize) '_regularized_recolored.dscalar.nii'],1,1,'default')
                
                %make_hierarchical_dendrogram([outfolder '/rawassn_minsize' num2str(infomap_minsize) '_regularized_allcolumns_recoloredv4.dscalar.nii'])
        ...
        if parcellation
            ...
            for seq = 1:length(functional_sequences)
                ...
                parcel_creator_cifti(['corrofcorr_allgrad_LR_subcort_smooth2.55_wateredge_avg.dtseries.nii'],[functional_sequences{seq} '_parcels'],parcel_borderthresh,ciftifile_normwall)
                make_cifti_label([functional_sequences{seq} '_parcels_edgethresh_' num2str(parcel_borderthresh) '.dtseries.nii'])
                ...
                subparcelfile = [functional_sequences{seq} '_parcels_edgethresh_' num2str(parcel_borderthresh) '.dlabel.nii'];
                
                [~,temp,~] = fileparts(subparcelfile);
                [~,parcellation_shortname,~] = fileparts(temp);
                mkdir([ciftifolder '/parcel_timecourses/'])
                parcel_timecourse_filename = [ciftifolder '/parcel_timecourses/' parcellation_shortname '.ptseries.nii'];
                system(['wb_command -cifti-parcellate ' ciftifile_normwall ' ' subparcelfile ' COLUMN ' parcel_timecourse_filename]);
                data = ft_read_cifti_mod(parcel_timecourse_filename);
                timecourses = data.data;
                data.dimord = 'chan_chan';
                data.data = paircorr_mod(data.data');
                data.data = FisherTransform(data.data);
                data.data(isnan(data.data)) = 0;
                ft_write_cifti_mod([correlationfolder '/' parcellation_shortname '_corr'],data);
                corrmat = data.data;
                save([correlationfolder '/' parcellation_shortname '_corr.mat'],'corrmat')
        ...
        if parcel_infomap
           ...
           %ign = rmdir(parcel_infomapfolder,'s');
            ...
            dlmwrite(logfile2,'running parcel infomap','-append','delimiter','')
...
%                 
                %if subnum==1
                    cifti_distance_matrix = '/home/data/scripts/Resources/Conte69_atlas-v2.LR.32k_fs_LR.wb/Conte69.LR.32k_fs_LR.distances_surfgeo_voleuc_normalwall_xhemlarge_standardsubcort_uint8.mat';
...
%                 
                 ...
                 consensus_maker_knowncolors([parcel_infomapfolder '/rawassn_minsize' num2str(parcel_infomap_minsize) '_regularized.dscalar.nii'],[],[],[],[],parcelfile)
...
%                 outname = [parcellationfolder '/' functional_sequences{seq}];
%                 Template_matching_cortexonly_parcels_onesub(cifti_timeseries,parcelfile,cifti_distance_matrix,parcel_infomap_xdist,.05,templatesfile,outname)
%                 tm_file = [outname '_templatematch_dice_kden' num2str(.05) '_parcels.dtseries.nii'];
%                 Hub_detection_summedPC_tm_degthresh(parcelfile,'corrmat.mat','parcel_distances.mat',tm_file,parcel_infomapthresholds,parcel_infomap_xdist);
 ...
 %               spring_embedding_func_easy_crossthresh([parcel_infomapfolder '/corrmat.mat'],[parcel_infomapfolder '/rawassn_minsize' num2str(parcel_infomap_minsize) '_regularized_recolored.txt'],1,25,[parcel_infomapfolder '/parcel_distances.mat'],parcel_infomap_xdist,[.01:.005:.05],[parcel_infomapfolder '/parcel_spring_embed'])
...
%         end
        ...
        cprintf('*blue',['Subject ' subject ' COMPLETED analysis without error!\n'])
