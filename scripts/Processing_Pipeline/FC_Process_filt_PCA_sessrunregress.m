function FC_Process_filt_PCA_sessrunregress(subject,BOLDlist,FDthresh,TR,outfolder,outprefix)
...
%     thisrun_params(:,1:3) = thisrun_rot_mm;
    ...
    thisrun_params_unfilt = load(motionparamsfiles{runnum});
...
title(string)
export_fig(gca,[outfolder '/' outprefix '_all_FD_filt.pdf'])

%Get run and session regressors
runregressors = repmat((1:nruns-1),length(runIDs),1);
runregressors = (runregressors==repmat(runIDs,1,size(runregressors,2)))';

uniquesessIDs = unique(sessionIDs); uniquesessIDs = uniquesessIDs(:)';
sessregressors = repmat(uniquesessIDs(1:end-1),length(sessionIDs),1);
sessregressors = (sessregressors==repmat(sessionIDs,1,size(sessregressors,2)))';
...
all_regressors = [motion_regressors ; physio_regressors ; physio_regressors_delta; runregressors ; sessregressors]';
