...
functional_sequences = {'restingstate'};%{'RSFC'};

functional_sequence_TRs = {2.2};%{2.0}; 

functional_sequence_rawsize = {[64 64 34]};%[80 80 34]
...
%                             save_untouch_nii(data,[BOLDrunnames{end} '.nii.gz'])
                            ...
                            copyfile(filename,[BOLDrunnames{end} '.nii.gz'])
                        end
        ...
        if DTI_preprocess
            ...
            for filenum = 1:DTIcounter
                ...
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' DTIfile '_ec_FA -interp spline -ref ' template ' -applyisoxfm ' num2str(normalized_voxdim) ' -init ' DTIfile '_2MNI.mat -out ' DTIfile '_ec_FA_MNI']);
            ...
            for seq = 1:length(functional_sequences)
                ...
                for runnum = 1:length(BOLDrunnames)
                    ...
                    if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                        ...
                        data = load_untouch_nii_2D([BOLDfile '.nii.gz']);
                        nvox = size(data.img,1);
                        padsize = 1000;
                        padded_data = [zeros(padsize,nvox) ; data.img' ; zeros(padsize,nvox)];
                        filtered = filtfilt(butta,buttb,double(padded_data));
                        data_filtered = filtered(1005:end-1004,:);
                        data.img = data_filtered';
                        data.hdr.dime.dim(5) = size(data.img,2);
                        save_untouch_nii_2D(data,[BOLDfile '_filt.nii.gz']);
...
%                         [systemresult{end+1,1},systemresult{end+1,2}] = system(['slicetimer -i ' BOLDfile ' -o ' BOLDfile '_filt -r ' num2str(TR) ' ' sliceorderinfo{seq}]);
%                         %copyfile([BOLDfile '.nii.gz'],[BOLDfile '_filt.nii.gz']);
%                         
%                         ign = rmdir([BOLDfile '_filt_mcf.mat*'],'s');
%                         %motion-correct BOLD images to first image to get motion plots
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['mcflirt -in ' BOLDfile '_filt -refvol 0 -mats -plots']);
                        
                        %split slice-time corrected timeseries into separate volumes
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslsplit ' BOLDfile '_filt ' BOLDfile '_filt_mcf.mat/' BOLDfilename '_filt_mcf -t']);
                        
                        %get avg BOLD for this run
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths ' BOLDfile '_filt_mcf -Tmean ' BOLDfile '_filt_mcf_avg']);
                        
                        %Add to string to concatenate avg BOLD images
                        avg_mergestr_native = [avg_mergestr_native ' ' BOLDfile '_filt_mcf_avg'];
                        
                        %delete motion corrected image--we will include this in a single-step atlas transformation below
                        delete([BOLDfile '_filt_mcf.nii.gz'])
                        
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['bet ' BOLDfile '_filt_mcf_avg ' BOLDfile '_filt_mcf_avg_bet -R']);
                        %[systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_filt_mcf_avg_bet -ref '  T1file '_bet -dof 12 -omat ' BOLDfile '_avg_2T1.mat']);
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_filt_mcf_avg_bet -ref '  T1file '_bet -dof 12 -omat ' BOLDfile '_avg_2T1_init.mat']);
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_filt_mcf_avg_bet -ref '  T1file '_bet -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' BOLDfile '_avg_2T1_init.mat -omat ' BOLDfile '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
                        ...
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_filt_mcf_avg -interp spline -ref ' template ' -applyisoxfm ' num2str(normalized_voxdim) ' -init ' BOLDfile '_avg_2MNI.mat -out ' BOLDfile '_filt_mcf_avg_MNI']);
                        
                        avg_mergestr_MNI = [avg_mergestr_MNI ' ' BOLDfile '_filt_mcf_avg_MNI'];
                        
                        %get volume numbers
                        vols = dir([BOLDfile '_filt_mcf.mat/MAT*']);
                        fslmergestr = ['fslmerge -t ' BOLDfile '_filt_mcf_MNI'];
                        %for each volume
                        for i = 1:length(vols)
                            %concatenate motion correction transform with run avg to MNI transform
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat ' BOLDfile '_filt_mcf.mat/' vols(i).name '_MNI.mat -concat  ' BOLDfile '_avg_2MNI.mat ' BOLDfile '_filt_mcf.mat/' vols(i).name]);
                            
                            %apply concatenated transform
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_filt_mcf.mat/' BOLDfilename '_filt_mcf' sprintf('%04i',(i-1)) ' -interp spline -ref ' template ' -applyisoxfm ' num2str(normalized_voxdim) ' -init '  BOLDfile '_filt_mcf.mat/' vols(i).name '_MNI.mat -out ' BOLDfile '_filt_mcf.mat/' BOLDfilename '_filt_mcf_MNI' sprintf('%04i',(i-1))]);
                            
                            %prepare to merge all volumes back into a 4D image
                            fslmergestr = [fslmergestr ' ' BOLDfile '_filt_mcf.mat/' BOLDfilename '_filt_mcf_MNI' sprintf('%04i',(i-1))];
                        end
                        %merge volumes
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(fslmergestr);
                        
                        %remove unneeded files
                        delete([BOLDfile '_filt_mcf.mat/*'])
                        rmdir([BOLDfile '_filt_mcf.mat'],'s')
                        delete([BOLDfile '_filt.nii.gz'])
                        delete([BOLDfile '_avg_2T1_init.mat'])
                        delete([BOLDfile '_filt_mcf_avg_bet.nii.gz'])
                        
                        
                        %Intensity normalization to mode within-brain value of 1000
                        data = load_untouch_nii([BOLDfile '_filt_mcf_MNI.nii.gz']);
                        ...
                        save_untouch_nii(data,[BOLDfile '_filt_mcf_MNI.nii.gz']);
                        data = []; data_inmask = [];
                        
                        %track session number
                        dlmwrite(sess_tracker,[BOLDfile '_filt_mcf_MNI.nii.gz ' num2str(sesscounter(runnum)) ' ' BOLDfile '_filt_mcf.par'],'-append','delimiter','');%write the data to the output file
                    end
                    ...
                    if fcprocess_sequences{seq}
                        ...
                        for runnum = 1:length(BOLDrunnames)
                            ...
                            if strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                                fslmergestr = [fslmergestr BOLDrunnames{runnum} '_filt_mcf_MNI.nii.gz '];
                            end
                        end
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(fslmergestr);
