...
%subjects = '/home/data/subjects/processing_list_011917.txt';
%subjects = '/home/data/subjects/list_forJDP.txt';
subjects = {'Movie001','Movie003','Movie006'};
...
%             sessions = sessions(3:end);
            ...
            for filenum = 1:DTIcounter
                ...
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' DTIfile '_ec_b0 -ref ' preprocfolder '/' T1file ' -dof 12 -omat ' DTIfile '_2T1_init.mat']);
        ...
        if BOLD_preprocess
            ...
            for seq = 1:length(functional_sequences)
                ...
                if seq==1
                    ...
                    bestrun_name = runnametracker{bestind};
                    try delete(['allBOLDavgs_' functional_sequences{seq} '_bet_mcf.mat/*'])
                        rmdir(['allBOLDavgs_' functional_sequences{seq} '_bet_mcf.mat'],'s')
                    catch; end
                    
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' bestrun_name '_st_mcf_avg_bet' fmstring ' -ref '  T1file ' -dof 6 -omat ' bestrun_name '_avg_2T1_init.mat']);
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' bestrun_name '_st_mcf_avg_bet' fmstring ' -ref '  T1file ' -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' bestrun_name '_avg_2T1_init.mat -omat ' bestrun_name '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
                    
                    %Make MNI registration
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat ' bestrun_name '_avg_2MNI.mat -concat T1_2MNI.mat ' bestrun_name '_avg_2T1.mat']);
                    
                end
                
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['mcflirt -in allBOLDavgs_' functional_sequences{seq} '_bet -reffile ' bestrun_name '_st_mcf_avg_bet -bins 10000 -mats']);
                [systemresult{end+1,1},systemresult{end+1,2}] = system(['applyxfm4D allBOLDavgs_' functional_sequences{seq} ' allBOLDavgs_' functional_sequences{seq} '_bet_mcf allBOLDavgs_' functional_sequences{seq} '_mcf allBOLDavgs_' functional_sequences{seq} '_bet_mcf.mat -userprefix MAT_']);
%                 alldata = load_untouch_nii_2D(['allBOLDavgs_' functional_sequences{seq} '_mcf.nii.gz']);
%                 corrs = paircorr_mod(double(alldata.img));
%                 corrs = corrs(:,bestind);
                ...
                for runnum = 1:length(BOLDrunnames)
                    ...
                    if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                        ...
                        %if acceptable(runnumtracker)
                            ...
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat ' BOLDfile '_avg_2MNI.mat -concat ' bestrun_name '_avg_2MNI.mat allBOLDavgs_' functional_sequences{seq} '_bet_mcf.mat/MAT_' sprintf('%04i',runnumtracker-1)]);
