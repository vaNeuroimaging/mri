...
%subjects = 'processing_list_GOBA_110615.txt';
...
for subnum = 1:subcount
    ...
    try
        ...
        if apply_fieldmap
            ...
            fieldmap_mag_mergestring = 'fslmerge -t fieldmap_mag_all ';
            fieldmap_phasediff_mergestring = 'fslmerge -t fieldmap_phasediff_all ';
            
            fieldmap_mergestring = 'fslmerge -t fieldmap_rads_T1space_all ';
            fieldmapmask_mergestring = 'fslmerge -t fieldmap_mag_bet_mask_T1space_all ';
            ...
            for sess = 1:length(sessions)
                ...
                for i = 1:length(FMfiles_thissess)
                    ...
                    if FMcounter > 1
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in fieldmap_' num2str(FMcounter) '_mag -ref fieldmap_1_mag -dof 6 -omat fieldmap_' num2str(FMcounter) '_2fieldmap_1.mat -out fieldmap_' num2str(FMcounter) '_mag_2fieldmap_1']);
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in fieldmap_' num2str(FMcounter) '_phasediff -ref fieldmap_1_mag -applyxfm -init fieldmap_' num2str(FMcounter) '_2fieldmap_1.mat -out fieldmap_' num2str(FMcounter) '_phasediff_2fieldmap_1']);
                        fieldmap_mag_mergestring = [fieldmap_mag_mergestring 'fieldmap_' num2str(FMcounter) '_mag_2fieldmap_1 '];
                        fieldmap_phasediff_mergestring = [fieldmap_phasediff_mergestring 'fieldmap_' num2str(FMcounter) '_phasediff_2fieldmap_1 '];
                    end
                end
            end
            
            [systemresult{end+1,1},systemresult{end+1,2}] = system(fieldmap_mag_mergestring);
            [systemresult{end+1,1},systemresult{end+1,2}] = system('fslmaths fieldmap_mag_all -Tmean fieldmap_mag_mean');
            [systemresult{end+1,1},systemresult{end+1,2}] = system(fieldmap_phasediff_mergestring);
            [systemresult{end+1,1},systemresult{end+1,2}] = system('fslmaths fieldmap_phasediff_all -Tmean fieldmap_phasediff_mean');
            
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['bet fieldmap_mag_mean fieldmap_mag_mean_bet -R']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in fieldmap_mag_mean_bet -ref ' T1file '_bet -dof 12 -omat fieldmap_2T1.mat']);
            
            betimage = load_untouch_nii(['fieldmap_mag_mean_bet.nii.gz']);
            betimage.img(:,:,1) = 0; betimage.img(:,:,end) = 0;
            save_untouch_nii(betimage,['fieldmap_mag_mean_bet.nii.gz']);
            
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths fieldmap_mag_mean_bet -ero fieldmap_mag_mean_bet']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths fieldmap_mag_mean_bet -ero fieldmap_mag_mean_bet']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths fieldmap_mag_mean_bet -ero fieldmap_mag_mean_bet']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths fieldmap_mag_mean_bet -bin fieldmap_mag_mean_bet_mask']);
            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fsl_prepare_fieldmap SIEMENS fieldmap_phasediff_mean fieldmap_mag_mean_bet fieldmap_rads ' num2str(fieldmap_deltaTE)]);
                ...
                for runnum = 1:length(BOLDrunnames)
                    ...
                    if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                        ...
                        if apply_fieldmap
                            ...
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths ' BOLDfile '_mcf -Tmean ' BOLDfile '_mcf_avg']);
                            
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['bet ' BOLDfile '_mcf_avg ' BOLDfile '_mcf_avg_bet -R']);
                            
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_mcf_avg_bet -ref '  T1file '_bet -dof 12 -omat ' BOLDfile '_avg_2T1.mat']);%'_avg_2T1_init.mat']);
                            %[systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_mcf_avg_bet -ref '  T1file '_bet -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' BOLDfile '_avg_2T1_init.mat -omat ' BOLDfile '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
                            
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat T1_2' BOLDfilename '.mat -inverse ' BOLDfile '_avg_2T1.mat']);
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat fieldmap_2' BOLDfilename '.mat -concat T1_2' BOLDfilename '.mat fieldmap_2T1.mat']);
                            
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in fieldmap_rads -interp spline -ref ' BOLDfile ' -applyxfm -init fieldmap_2' BOLDfilename '.mat -out fieldmap_rads_' BOLDfilename]);
                            
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['fugue -i ' BOLDfile ' --dwell=' num2str(echospacing) ' --unwarpdir=y- --loadfmap=fieldmap_rads_' BOLDfilename ' -u ' BOLDfile '_uwrp']);
                            
                            delete([BOLDfile '_mcf.nii.gz'])
                            delete([BOLDfile '_mcf_avg.nii.gz'])
                            delete([BOLDfile '_mcf_avg_bet.nii.gz'])
                            delete([BOLDfile '_avg_2T1_init.mat'])
                            %delete(['fieldmap_2' BOLDfilename '.mat'])
                            %delete(['T1_2' BOLDfilename '.mat'])
                            
                            movefile([BOLDfile '_mcf.par'],[BOLDfile '_uwrp_mcf.par']);
                            BOLDfile = [BOLDfile '_uwrp'];
                            BOLDfilename = [BOLDfilename '_uwrp'];
                        ...
                        %[systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet -ref '  T1file '_bet -dof 12 -omat ' BOLDfile '_avg_2T1.mat']);
        ...
        if BOLD_fcprocess
            ...
            for seq = 1:length(functional_sequences)
                if fcprocess_sequences{seq}
                    ...
                    FC_Process_filt(subject,['/home/data/subjects/' subject '/preprocessed/BOLDruns_sessions_' functional_sequences{seq} '.txt'],FDthresh,functional_sequence_TRs{seq},['/home/data/subjects/' subject '/fc_processed/'],functional_sequences{seq})
