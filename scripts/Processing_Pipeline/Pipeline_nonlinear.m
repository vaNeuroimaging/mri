...
%subjects = {'MAV008'};
%subjects = {'MAV033','MAV036','MAV020'};
subjects = {'MAV036','MAV020'};
...
functional_sequences = {'RSFC'};%{'restingstate'}
...
for subnum = 1:subcount
        ...
        for seq = 1:length(functional_sequences)
            ...
            for runnum = 1:length(BOLDrunnames)
                ...
                if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                    ...
                    if firstrun
                        [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslselectvols -i ' BOLDfile '_st -o Volume1 --vols==0']);
                        firstrun = false;
                    end
                    
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['mcflirt -in ' BOLDfile '_st -reffile Volume1']);
                    try delete('temp_fnirt/*')
                        rmdir('temp_fnirt','s')
                    catch; end
                    mkdir('temp_fnirt')
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslsplit ' BOLDfile '_st_mcf temp_fnirt/']);
                    files = dir('temp_fnirt/*.nii.gz');
                    parfor i = 1:length(files)
                        [~,~] = system(['fnirt --ref=Volume1 --in=temp_fnirt/' files(i).name(1:end-7) ' --iout=temp_fnirt/' files(i).name(1:end-7) '_warped']);
                    end
                    fslmergestr = ['fslmerge -t ' BOLDfile '_st_mcf_fnirt'];
                    for i = 1:length(files)
                        fslmergestr = [fslmergestr ' temp_fnirt/' files(i).name(1:end-7) '_warped'];
                    end
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(fslmergestr);
                    
                    
                    %get avg BOLD for this run
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['fslmaths ' BOLDfile '_st_mcf_fnirt -Tmean ' BOLDfile '_st_mcf_fnirt_avg']);
                    ...
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['bet ' BOLDfile '_st_mcf_fnirt_avg ' BOLDfile '_st_mcf_fnirt_avg_bet -R']);
                    ...
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_fnirt_avg_bet -ref '  T1file '_bet -dof 12 -omat ' BOLDfile '_avg_2T1_init.mat']);
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_fnirt_avg_bet -ref '  T1file '_bet -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' BOLDfile '_avg_2T1_init.mat -omat ' BOLDfile '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
                    ...
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_fnirt_avg -interp spline -ref ' template ' -applyisoxfm ' num2str(normalized_voxdim) ' -init ' BOLDfile '_avg_2MNI.mat -out ' BOLDfile '_st_mcf_fnirt_avg_MNI']);
                    [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_fnirt -interp spline -ref ' template ' -applyisoxfm ' num2str(normalized_voxdim) ' -init ' BOLDfile '_avg_2MNI.mat -out ' BOLDfile '_st_mcf_fnirt_MNI']);
                   ...
                   %remove unneeded files
                    ...
                    delete([BOLDfile '_st_mcf_fnirt_avg_bet.nii.gz'])
                    
                    
                    %Intensity normalization to mode within-brain value of 1000
                    data = load_untouch_nii([BOLDfile '_st_mcf_fnirt_MNI.nii.gz']);
                    ...
                    save_untouch_nii(data,[BOLDfile '_st_mcf_fnirt_MNI.nii.gz']);
                    data = []; data_inmask = [];
                    
                    
                    %Add to string to concatenate avg BOLD images
                    avg_mergestr_native = [avg_mergestr_native ' ' BOLDfile '_st_mcf_fnirt_avg'];
                    avg_mergestr_MNI = [avg_mergestr_native ' ' BOLDfile '_st_mcf_fnirt_avg_MNI'];
                    
                    %track session number
                    dlmwrite(sess_tracker,[BOLDfile '_st_mcf_fnirt_MNI.nii.gz ' num2str(sesscounter(runnum)) ' ' BOLDfile '_mcf.par'],'-append','delimiter','');%write the data to the output file
                end
    ...
    disp(['Subject ' subject ' COMPLETED processing without error!'])
