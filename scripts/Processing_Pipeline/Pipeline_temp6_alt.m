...
%subjects = '/home/data/subjects/processing_list_092616.txt';
...
%             sessions = sessions(3:end);
                    ...
                    if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                        ...
                        if apply_fieldmap
                            ...
                            if isempty(firstrun_name{seq})
                                ...
                                %Register first warped average BOLD image from first session to T1 via two-step boundary-based registration 
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet_uwrp -ref '  T1file '_bet -dof 6 -omat ' BOLDfile '_avg_2T1_init.mat']);
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet_uwrp -ref '  T1file '_bet -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' BOLDfile '_avg_2T1_init.mat -omat ' BOLDfile '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch']);
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat T1_2' BOLDfilename '_avg.mat -inverse ' BOLDfile '_avg_2T1.mat']);
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in aparc+aseg_cerebralwm -applyxfm -init T1_2' BOLDfilename '_avg.mat -ref ' BOLDfile '_st_mcf_avg_bet_uwrp -interp nearestneighbour -out aparc+aseg_cerebralwm_' BOLDfilename 'space']);
                            ...
                            %Register this session warped average BOLD image to first session warped average BOLD image (for consistency, this is done for session 1 as well)
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet_uwrp -ref '  firstrun_name{seq} '_st_mcf_avg_bet_uwrp -cost normcorr -dof 6 -omat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg_init.mat']);
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet_uwrp -ref '  firstrun_name{seq} '_st_mcf_avg_bet_uwrp -cost normcorr -dof 6 -cost bbr -wmseg aparc+aseg_cerebralwm_' firstrun_name{seq} 'space -init ' BOLDfile '_avg_2' firstrun_name{seq} '_avg_init.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch -omat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg.mat']);
                            ...
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_uwrp -interp spline -ref ' template ' -applyisoxfm ' num2str(normalized_voxdim) ' -init ' BOLDfile '_avg_2MNI.mat -out ' BOLDfile '_st_mcf_avg_MNI']);
                            ...
                            if isempty(firstrun_name{seq})
                                ...
                                %Register first average BOLD image from first session to T1 via two-step boundary-based registration 
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet -ref '  T1file '_bet -dof 6 -omat ' BOLDfile '_avg_2T1_init.mat']);
                                ...
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat T1_2' BOLDfilename '_avg.mat -inverse ' BOLDfile '_avg_2T1.mat']);
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in aparc+aseg_cerebralwm -applyxfm -init T1_2' BOLDfilename '_avg.mat -ref ' BOLDfile '_st_mcf_avg_bet -interp nearestneighbour -out aparc+aseg_cerebralwm_' BOLDfilename 'space']);
                                
                                %Make MNI registration
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat ' BOLDfile '_avg_2MNI.mat -concat T1_2MNI.mat ' BOLDfile '_avg_2T1.mat']);
                            end
                            
                            %Register this session average BOLD image to first session average BOLD image (for consistency, this is done for session 1 as well)
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet -ref '  firstrun_name{seq} '_st_mcf_avg_bet -cost normcorr -dof 6 -omat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg_init.mat']);
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet -ref '  firstrun_name{seq} '_st_mcf_avg_bet -cost normcorr -dof 6 -cost bbr -wmseg aparc+aseg_cerebralwm_' firstrun_name{seq} 'space -init ' BOLDfile '_avg_2' firstrun_name{seq} '_avg_init.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch -omat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg.mat']);
                            
                            %Make MNI registration
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['convert_xfm -omat ' BOLDfile '_avg_2MNI.mat -concat  ' firstrun_name{seq} '_avg_2MNI.mat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg.mat']);
                            
                            %Register session average BOLD image to MNI
        ...
        fprintf(2,['Subject ' subject ' FAILED to complete processing! Check error log ' logfile '\n'])
...
for subnum = 1:subcount
    ...
    try
        ...
        fprintf(2,['Subject ' subject ' FAILED to complete post-processing! Check error log ' logfile2 '\n'])
