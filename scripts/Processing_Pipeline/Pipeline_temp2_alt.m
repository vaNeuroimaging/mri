...
%subjects = 'processing_list_092216.txt';
...
for subnum = 1:subcount
    ...
    try
        ...
        if apply_fieldmap
            ...
            [systemresult{end+1,1},systemresult{end+1,2}] = system('fslmaths fieldmap_rads_T1space_all -Tmean fieldmap_rads_T1space_mean');
            
            [systemresult{end+1,1},systemresult{end+1,2}] = system(fieldmapmask_mergestring);
            [systemresult{end+1,1},systemresult{end+1,2}] = system('fslmaths fieldmap_mag_bet_mask_T1space_all -Tmean -thr 1 fieldmap_mag_bet_mask_T1space_union');
            
            [systemresult{end+1,1},systemresult{end+1,2}] = system('fslmaths fieldmap_rads_T1space_mean -mul fieldmap_mag_bet_mask_T1space_union fieldmap_rads_T1space_mean');
                ...
                for runnum = 1:length(BOLDrunnames)
                    ...
                    if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                        ...
                        if isempty(firstrun_name{seq})
                            ...
                            if apply_fieldmap
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet -ref '  T1file '_bet -dof 12 -cost bbr -wmseg aparc+aseg_cerebralwm -init ' BOLDfile '_avg_2T1_init.mat -omat ' BOLDfile '_avg_2T1.mat -schedule ${FSLDIR}/etc/flirtsch/bbr.sch -fieldmap fieldmap_rads_T1space_mean -fieldmapmask fieldmap_mag_bet_mask_T1space_union -pedir -2 -echospacing ' num2str(echospacing)]);
                            else
                            ...
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg -ref '  firstrun_name{seq} '_st_mcf_avg -dof 6 -omat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg.mat']);
                        ...
                        if strcmp(firstrun_name{seq},BOLDfilename)
                            datafirst = data.img;
                        elseif paircorr_mod(datafirst,data.img) < .98
        ...
        if BOLD_fcprocess
            ...
            for seq = 1:length(functional_sequences)
                if fcprocess_sequences{seq}
                    ...
                    FC_Process_filt_PCA(subject,['/home/data/subjects/' subject '/preprocessed/BOLDruns_sessions_' functional_sequences{seq} '.txt'],FDthresh,functional_sequence_TRs{seq},['/home/data/subjects/' subject '/fc_processed/'],functional_sequences{seq})
