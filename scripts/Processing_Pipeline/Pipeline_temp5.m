            ...
            for seq = 1:length(functional_sequences)
                ...
                for runnum = 1:length(BOLDrunnames)
                    ...
                    if (length(BOLDfilename) >= length(functional_sequences{seq})) && strcmp(BOLDfilename(1:length(functional_sequences{seq})),functional_sequences{seq});
                        ...
                        fieldmap_exist = false;
                        if exist([this_fieldmaprads '.nii.gz'])
                            fieldmap_exist = true;
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' this_fieldmapmag ' -ref ' BOLDfile '_st_mcf_avg_bet -dof 6 -omat fieldmap_sess' num2str(sesscounter(runnum)) '_2' BOLDfilename '.mat']);
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' this_fieldmaprads ' -ref ' BOLDfile '_st_mcf_avg_bet -applyxfm -init fieldmap_sess' num2str(sesscounter(runnum)) '_2' BOLDfilename '.mat -out ' this_fieldmaprads '_' BOLDfilename 'space']);
                            [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' this_fieldmapmask ' -ref ' BOLDfile '_st_mcf_avg_bet -applyxfm -init fieldmap_sess' num2str(sesscounter(runnum)) '_2' BOLDfilename '.mat -interp nearestneighbour -out ' this_fieldmapmask '_' BOLDfilename 'space']);
                        end
                        
                        if isempty(firstrun_name{seq})
                            firstrun_name{seq} = BOLDfilename;
                            if apply_fieldmap && fieldmap_exist
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['fugue -i ' BOLDfile '_st_mcf_avg_bet --dwell=' num2str(echospacing) ' --loadfmap=' this_fieldmaprads '_' BOLDfilename 'space --mask=' this_fieldmapmask '_' BOLDfilename 'space -u ' BOLDfile '_st_mcf_avg_bet_uwrp']);
                                ...
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_bet_uwrp -ref '  T1file '_bet -dof 12 -omat ' BOLDfile '_avg_2T1_init.mat']);
                            ...
                            if apply_fieldmap && fieldmap_exist && exist([firstrun_name{seq} '_st_mcf_avg_uwrp.nii.gz'])
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['fugue -i ' BOLDfile '_st_mcf_avg_bet --dwell=' num2str(echospacing) ' --loadfmap=' this_fieldmaprads '_' BOLDfilename 'space --mask=' this_fieldmapmask '_' BOLDfilename 'space -u ' BOLDfile '_st_mcf_avg_bet_uwrp']);
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['fugue -i ' BOLDfile '_st_mcf_avg --dwell=' num2str(echospacing) ' --loadfmap=' this_fieldmaprads '_' BOLDfilename 'space --mask=' this_fieldmapmask '_' BOLDfilename 'space -u ' BOLDfile '_st_mcf_avg_uwrp']);
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['flirt -in ' BOLDfile '_st_mcf_avg_uwrp -ref '  firstrun_name{seq} '_st_mcf_avg_uwrp -cost normcorr -dof 6 -omat ' BOLDfile '_avg_2' firstrun_name{seq} '_avg.mat']);
                            else
...
%                             [systemresult{end+1,1},systemresult{end+1,2}] = system(fslmergestr);
                            ...
                            if apply_fieldmap && fieldmap_exist
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['fugue -i ' BOLDfile '_st --dwell=' num2str(echospacing) ' --loadfmap=' this_fieldmaprads ' --mask=' this_fieldmapmask ' -u ' BOLDfile '_st_uwrp']);
                                %apply concatenated transforms
                                [systemresult{end+1,1},systemresult{end+1,2}] = system(['applyxfm4D ' BOLDfile '_st_uwrp ' BOLDfile '_st_mcf_avg_MNI ' BOLDfile '_st_mcf_MNI ' BOLDfile '_st_mcf.mat/ -userprefix Native2MNI_MAT_']);
                            else
