function Hub_detection_summedweightedPC(parcelsfile,corrmatfile,dmatfile,communitiesfile,consensus_modulesfile,kdenthresholds,xdist)
...
pctconnections_needed_tocount_foracommunity = .05;
...
for threshnum = 1:length(kdenthresholds)
    ...
    corrmat_thresh = corrmat .* (corrmat > 0);%rthresh);
    corrmat_bin_all(:,:,threshnum) = corrmat_thresh;
    
    thresh_communities = unique(communities(:,threshnum));
    thresh_communities(thresh_communities<1) = [];
    
    parcel_degree = sum(corrmat_thresh,2);
    moduleconnectionssum = zeros(size(corrmat_thresh,1),1);
    ...
    for communitynum = 1:length(thresh_communities)
        ...
        parcel_community_degree = sum(corrmat_thresh(:,communityindices),2);
...
corrmat_bin_sum = sum(logical(corrmat_bin_all),3);
...
save('summed_wPCs.mat','summed_PCs')
...
ft_write_cifti_mod('parcel_wPCs_percentile',cifti_template)  
...
ft_write_cifti_mod('parcel_wConnectorHubs',cifti_template)
...
ft_write_cifti_mod('parcel_wConnectorHubs_connections',stripedconnectors)
