function Hub_detection(parcelsfile,corrmatfile,dmatfile,modulesfile)

PC_cutoff_percentiles = [.5 : .05 : .9]; 
Deg_cutoff_percentiles = [];%PC_cutoff_percentiles;%repmat(.01, size(PC_cutoff_percentiles));
...
parcel_connectorhubs_map = zeros(length(parcels),length(PC_cutoff_percentiles));
PCthresh = zeros(1,length(PC_cutoff_percentiles));
Degthresh = zeros(1,length(Deg_cutoff_percentiles));

for i = 1:length(PC_cutoff_percentiles)
    ...
    cifti_template.mapname{i} = ['%tile thresh: PC=' num2str(PC_percentile) '; Degree=' num2str(Deg_percentile)];
...
%         parcel_connectorhubs_connections_mapHub_detection(parcels==parcelIDs(parcel),1:length(thisparcel_connections)) = repmat(thisparcel_connections(:)',[nnz(parcels==parcelIDs(parcel)),1]);
%     end
% end
...
% stripedconnectors.data = zeros(size(temp.data,1),length(PC_cutoff_percentiles));
% stripedconnectors.data(:,1) = temp.data;
% 
% for i = 2:(length(PC_cutoff_percentiles))
%     for parcel = 1:length(parcelIDs)
