function consensus_maker_knowncolors_alt(regularized_ciftifile,manualset,groupnetworksfile,mincol,minsize,orig_parcelsfile,recolor)
...
for c = 1:size(all_recolored,2)
    ...
    if exist('manualset') && ~isempty(manualset)
    ...
    for i = 1:length(potential_colors)
        ...
        if exist('manualset')  && ~isempty(manualset) && any(manualset(:,2)==potential_colors(i))
