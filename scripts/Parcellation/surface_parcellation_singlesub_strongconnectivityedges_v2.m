function  surface_parcellation_singlesub_strongconnectivityedges_v2(subject,dconnfile,distances,subsample,dosubcort,outputdir)
...
% that the gradient will be run on 1/10th of similarity maps, randomly
...
randinds = sort(randinds,'descend');


if ischar(distances)
    distances = smartload(distances);
end
closenough_verts = distances < networknearbydist;
closenough_verts_small = closenough_verts(:,randinds);
clear distances
...
sorted = sort(cifti_corrmap(triu(true(size(cifti_corrmap)),1)),'descend');
rthresh = sorted(ceil(size(cifti_corrmap,1) .* (size(cifti_corrmap,1)-1) ./ 2 .* kden));
clear sorted;
thresholded_corrmat = cifti_corrmap(:,randinds) >= rthresh;
...
if ~dosubcort
    ...
    thresholded_corrmat = thresholded_corrmat(1:ncortverts,:);
    closenough_verts = closenough_verts(1:ncortverts,1:ncortverts);
    closenough_verts_small = closenough_verts_small(1:ncortverts,:);
end
...
ft_write_cifti_mod([outputdir '/' gradsname '_smooth' num2str(smooth) '_wateredge'],ciftistruct);




edges_touse = false(size(edges));
for vertnum = 1:size(edges_touse,1)
    edges_touse(vertnum,:) = any(thresholded_corrmat(closenough_verts(vertnum,:),:),1);
end



% Average across gradient maps and save
edge_density = sum((edges.*(~closenough_verts_small).*edges_touse),2) ./ sum((~closenough_verts_small).*edges_touse,2);
