function  surface_parcellation_singlesub_strongconnectivityedges(subject,dconnfile,distances,dosubcort,outputdir)
...
distances = distances(1:size(cifti_corrmap,1),randinds);
...
clear sorted;
thresholded_corrmat = cifti_corrmap >= rthresh;
...
edge_density = sum((edges.*(~closenough_verts).*edges_touse),2) ./ sum((~closenough_verts).*edges_touse,2);
