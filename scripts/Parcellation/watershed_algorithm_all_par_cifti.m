...
for j = 1:divisions
    ...
    parfor l = labelstodo%size(labels,2)
        ...
        [~, count] = system('cat countingDir/countfile | wc -l');
        disp(['Calculating edges on map # ' num2str(count)])
        %fprintf([repmat('\b',1,stringlength) '%s'],['Calculating edges on metric #' sprintf('%05i',count)]);
        label = labels(:,l);
