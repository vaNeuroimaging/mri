function generate_rotated_parcellation_andPCA_nobaddata_generic(parcelsname,iterations,cov_corr,rotations,datainds,hem,outfileprefix,baddata)
%generate_rotated_parcellation_andPCA_nobaddata_generic(parcelsname,iterations,cov_corr,rotations,datainds,hem,outfileprefix,[baddataname])
...
realparcels = gifti(parcelsname); realparcels = realparcels.cdata;
...
parcelIDs = unique(realparcels); parcelIDs(parcelIDs==0) = [];
for parcelID = parcelIDs'
    if nnz(realparcels==parcelID) < (nnz(realparcels(allbadvertices)==parcelID) + minsize)
        realparcels(realparcels==parcelID) = 0;
    end
end


disp(['Evaluating homogeneities of parcels in ' parcelsname])

PCAvals = parcel_homogeneity_PCA_cov_generic(realparcels,cov_corr,datainds);
parcelsslashloc = strfind(parcelsname,'/');
if isempty(parcelsslashloc)
    parcelsslashloc = 0;
end

save(gifti(single(PCAvals)),['PCA_eigval_per_first_' parcelsname(parcelsslashloc(end)+1:end)])


parcelIDs = unique(realparcels); parcelIDs(parcelIDs==0) = [];

numparcels = nnz(unique(realparcels));
...
for parcelnum = 1:numparcels
    ...
    realparcelindices = find(realparcels==parcelID);
    ...
    maxdist = max(geo_distances(centerind,realparcelindices));
        ...
        string = ['Rotation ' num2str(iternum) ': parcel ' num2str(parcelnum)];
...
parcels = unique(realparcels);
parcels(parcels==0) = [];
templabel = zeros(size(realparcels));
for parcelsnum = 1:length(parcels)
    parcels = parcels(parcelsnum);
    templabel(realparcels==parcels) = parcelsnum;
    realsizes(parcelsnum) = nnz(realparcels==parcels);
    realeigvals_per_first(parcelsnum) = mean(PCAvals(templabel==parcelsnum));
