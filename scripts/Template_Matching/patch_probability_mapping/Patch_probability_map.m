function Patch_probability_map(patch_probability_map_params)
...
if new_cluster
    ...
    for s = 1:size(subnetworks,2)
        ...
        for ID = IDs(:)'
            ...
            clustereddata = cifti_cluster_surfacearea_volume(subnetworks(:,s),ID-.05,ID+.05,0,0,ncortverts,surfacearea,neighbors);
            ...
            for clusternum = 1:length(unique_clusters)
                ...
                cluster_surfacearea = sum(surfacearea(clustereddata==cluster_ID));
