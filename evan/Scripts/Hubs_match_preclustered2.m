function [matches, groupclus_dmat] = Hubs_match_preclustered2(groupclustermap,groupclusternetworkIDs, distances, groupclus_dmat,subclustermap,subclusternetworkIDs,distthresh)
...
for groupclusnum = 1:length(groupclusterIDs)
    for subclusnum = 1:length(subclusterIDs)
        ...
        if (length(commonconnections) == length(intersect(groupclusternetworkIDs{groupclusnum},common_networks))) && ...
