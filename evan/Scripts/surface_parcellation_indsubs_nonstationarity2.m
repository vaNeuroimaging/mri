...
surfdatadir = '/data/cn4/evan/RestingState/FC_Mapping_120/';
outputdir = '/data/cn4/evan/RestingState/FC_Mapping_120/NonstationarityParcellation2/';
...
for hem = h
    ...
    for s = 100%:length(subjects)
        ...
        for windownum = 1:numwindows
            ...
            avgcrosscorr = FisherTransform(paircorr_mod(single(surf_BOLD(:,windowindices))',single(vol_BOLD(windowindices,:))));
            ...
            maskout = maskout + isnan(avgcrosscorr(1,:));
            ...
            filestem = [subject '_avgcorrofcorr_smooth' num2str(smoothnum) '_allgrad_' HEMS{hem}];
            ...
            % Calculate and save average gradient map
            avg_metric = mean(grad_metrics,2);
            save(gifti(avg_metric),[filestem '_avg_window' num2str(windownum) '.func.gii']);
            
            % Calculate edge maps
            switch edges
                case 'no'
                case 'yes'
                    surface_edges_all_test_faster(grad_metrics,specfile,outputdir,[filestem '_edge_avg_window' num2str(windownum)],1);
            end
        end
    end
end