...
for hemnum = 1:length(hems)
    ...
    datalist = '/data/hcp-bluearc/home/laumannt/Poldrome/shared_for_washu/FCPROCESS_SCRUBBED_meanfield_lininterp_222/gradients_cifti_voxelclean_erode_concat_wateredge/all64/cohortfile.txt';
    tmasklist = '/data/hcp-bluearc/home/laumannt/Poldrome/shared_for_washu/FCPROCESS_SCRUBBED_meanfield_lininterp_222/gradients_cifti_voxelclean_erode_concat_wateredge/all64/tmaskfile.txt';
    iscifti = 2;
    %hem = 'R';
    ...
    outputfolder = ['/data/cn4/evan/RestingState/Ind_variability/Poldrome/Session_clustering/'];
    parcelfilename = ['/data/cn4/evan/RestingState/Ind_variability/Poldrome/Poldrome_' hem '_crossthresh_watershedmerge.func.gii'];
    ...
    totaltimeseries = '/data/cn4/evan/RestingState/Ind_variability/Poldrome/allsubs_LR_timeseries.func.gii';
    totaltmask = '/data/cn4/evan/RestingState/Ind_variability/Poldrome/allsubs_total_tmask.txt';
...
%    [subjects subdata] = textread(datalist,'%s%s');
    ...
    for subnum = 1:length(subjects)
        subdata{subnum} = ['/data/hcp-bluearc/home/laumannt/Poldrome/shared_for_washu/FCPROCESS_SCRUBBED_meanfield_lininterp_222/cifti_timeseries_voxelclean_erode_concat_wateredge/' subjects{subnum} '_BOLD_LR_surf_subcort_32k_fsLR_smooth2.55.dtseries.nii'];
    end
    ...
    try load([outputfolder 'parcelcorrelpatterns_' hem '.mat'])
    catch
    ...
    %save([outputfolder '264correlpatterns_' hem '.mat'],'parcelcorrelpatterns','-v7.3')
...
%     end
    ...
    %%
    finaloutput = zeros(length(parcels_upsampled),1);
    multiclusteroutput = zeros(length(parcels_upsampled),4);
    cophenet_output = zeros(length(parcels_upsampled),1);
    clusterconnections = zeros(length(parcelIDs),6);
    clustersizes = zeros(length(parcelIDs),6);
    maxQval = zeros(length(parcelIDs),1);
    for parcelnum = 1:length(parcelIDs)
        ...
        [H,T,perm] = dendrogram(clustering, 0, 'orientation','left', 'colorthreshold', .4);         % 'dendrogram' creates the tree
        ...
        clusters = cluster(clustering, 'MaxClust', [1:50]);
        ...
        [maxQval(parcelnum) maxQind] = max(Qvals);
        ...
        lines_within_parcel(lines_within_parcel==0)=[];
            ...
            for comnum = 1:length(communities)
                tempsizes(comnum) = nnz(thisthresh_clusters==communities(comnum));
            end
            if any(tempsizes>=networksizeminimum)            
                for comnum = 1:length(communities)
                ...
                [ign maxi] = max(tempsizes);
                newclusters = zeros(size(thisthresh_clusters));
                newclusters(thisthresh_clusters==communities(maxi)) = communities(maxi);
                thisthresh_clusters = newclusters;
            end
            ...
            for comnum = 1:length(communities)
                ...
                for networknum = 1:size(networkcorrelpattern,2)
                    indices = networkcorrelpattern(:,networknum) > 0;
            ...
            clusterconnections(parcelnum,1:length(communitysize)) = communityconnections(sortorder);
            clustersizes(parcelnum,1:length(communitysize)) = communitysize(sortorder);
%             for comnum = 1:length(communities)
%                 if comnum <= 4; multiclusteroutput((parcels_upsampled==parcelIDs(parcelnum)),comnum) = communityconnections(sortorder(comnum)); end
%             end
            %
        ...
        %Q_output(parcels_upsampled==parcelIDs(parcelnum)) = maxQval(parcelnum);
        end
    
     save(gifti(single(finaloutput)),['Parcel_networkIDs_subjectvariability_UPGMA_maxQ_minsize_' num2str(networksizeminimum) '_' hem '_164_alt.func.gii']);
     save(['Parcel_networkIDs_subjectvariability_UPGMA_maxQ_minsize_' num2str(networksizeminimum) '_' hem '_164_alt.mat'],'parcelIDs','clusterconnections','clustersizes','maxQval')
     
     
     %save(gifti(single(multiclusteroutput)),['Parcel_networkIDs_subjectvariability_UPGMA_maxQ_orderedclusters_minsize_' num2str(networksizeminimum) '_' hem '_164_alt.func.gii']);
     %save(gifti(single(cophenet_output)),['Parcel_networkIDs_subjectvariability_UPGMA_copheneticR_' hem '_164.func.gii']);
     %save(gifti(single(Q_output)),['Parcel_networkIDs_subjectvariability_UPGMA_maxQ_' hem '_164.func.gii']);
     
%     save(gifti(single(finaloutput)),['264_networkIDs_subjectvariability_UPGMA_maxQ_minsize_' num2str(networksizeminimum) '_' hem '_164.func.gii']);
%     save(gifti(single(cophenet_output)),['264_networkIDs_subjectvariability_UPGMA_copheneticR_' hem '_164.func.gii']);
%     save(gifti(single(Q_output)),['264_networkIDs_subjectvariability_UPGMA_maxQ_' hem '_164.func.gii']);        
