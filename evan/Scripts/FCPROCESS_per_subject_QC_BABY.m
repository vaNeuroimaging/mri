function [allends switches] = FCPROCESS_per_subject_QC_BABY(datalist, tdir, tmasktype, cleanup, roilist, freesurfer_datapath_file, varargin)
...
run_spec = input('Is this your initial(1) or final(2) run of FCPROCESS? : ');

if run_spec == 1
    disp ('Moving on with default parameters...')
    ...
    switches.plot_silent = 1;
    
    if switches.plot_silent == 1
        set(0, 'DefaultFigureVisible', 'off');
    elseif swithces.plot_silent == 0
        set(0, 'DefaultFigureVisible', 'on');
    end
...
needcorrectinput = 1;
while needcorrectinput
    switches.plot_silent = input('\nWould you like plots to be in the foreground or background while analysis runs?  (0 = foreground, 1 = background) ' );
    if switches.plot_silent == 1
        set(0, 'DefaultFigureVisible', 'off');
    elseif switches.plot_silent == 0
        set(0, 'DefaultFigureVisible', 'on');
    end
    needcorrectinput=0;
end
...
% Have to specifiy these explcitly since they were initially done jankily ...
if switches.doblur
    blursize=2*log(2)/pi*10/switches.blurkernel;
end
if switches.plot_silent == 1
...
fprintf('switches.plot_silent (1 = silent; 0 = plot data): %d\n', switches.plot_silent)
...
roimasks = read_4dfpimg_HCP(rois{1});

%% CHECK THAT BASIC FILES ARE ACTUALLY THERE
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % Basic modifications made such that QC.mat's can be made individually
% % if isdir(datalist)
% %     subject_list = dir([datalist '/*.txt']);    
% % else
% %     disp('The first parameter needs to be a folder')
% % end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 

[df_all.filepath df_all.vcnum df_all.prmfile df_all.TR df_all.TRskip] = textread(datalist,'%s%s%s%f%d');   

if strcmp(tmasktype, 'ones')
    %don nothing
else
    %             cd(startdir)
    %             tm.vcnum = tm_all.vcnum{count};
    %             tm.tmaskfiles = tm_all.tmaskfiles{count};
    ...
    [tm_all.vcnum tm_all.tmaskfiles]=textread(tmasktype,'%s%s');
    % % need to figure out the logic post intial_process for singular QC.mat...
    if ~isequal(tm_all.vcnum,df_all.vcnum)
        error('masklist vcnumbers do not match datalist vcnumbers');
    end
end
...
for count = 1:length(df_all.vcnum)
    
    try
    
    clearvars -except count failed_subjects failed_subjects_count failed_subjects_err err_count FDoutlier GLMoutlierlim GREYoutlierlim ROIreadTime_ID avidir cleanup cont datalist releasedir roidir ...
        roilist roimasks rois run_spec startdir subject_list switches tdir tmasktype varargin voxdim freesurfer_datapath_file blursize df_all tm_all % clear out the prevous subject's QC
    % read in the subject data including location, vcnum, and boldruns
%     [df.filepath df.vcnum df.prmfile df.TR df.TRskip] = textread([datalist '/' subject_list(count).name], '%s%s%s%f%d');
    df.filepath = df_all.filepath(count);
    df.vcnum = df_all.vcnum(count);
    df.prmfile = df_all.prmfile(count);
    df.TR = df_all.TR(count);
    df.TRskip = df_all.TRskip(count);
...
%% READ IN FREESURFER
if ~isempty(freesurfer_datapath_file)
    [FSfile.vc FSfile.dir]=textread(freesurfer_datapath_file,'%s%s');
end
errorout=0;
for i=1:numsubs
    found=0;
...
for i=1:numsubs
    ...
    for j=1:numel(QC(i).restruns)
        ...
        % set original data
        boldmat{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/bold' num2str( QC(i).restruns(j)) '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d.mat' ];
        boldimg{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/bold' num2str( QC(i).restruns(j)) '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d_atl.4dfp.img' ];
        boldifh{i,j} = [ df.filepath{i,1} '/' QC(i).vcnum '/bold' num2str( QC(i).restruns(j)) '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d_atl.4dfp.ifh' ];
    ...
    anataveimg{i,1} = [ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_anat_ave_on_711-2N_333.4dfp.img'];
    anataveifh{i,1} = [ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_anat_ave_on_711-2N_333.4dfp.ifh'];
    
    mprimg{i,1}=ls([ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_mpr_on_711-2N_333.4dfp.img']);
    mprifh{i,1}=ls([ df.filepath{i,1} '/' QC(i).vcnum '/atlas/' QC(i).vcnum '_mpr_on_711-2N_333.4dfp.ifh']);
    mprimg{i,1}(end)=[];
    mprifh{i,1}(end)=[];
...
for i=1:numsubs
    ...
    tanataveimg{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_anat_ave_on_711-2N_333.4dfp.img'];
    tanataveifh{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_anat_ave_on_711-2N_333.4dfp.ifh'];
    system([ 'ln -s ' anataveimg{i,1} ' ' tanataveimg{i,1}]);
    system([ 'ln -s ' anataveifh{i,1} ' ' tanataveifh{i,1}]);
    
    tmprimg{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_mpr_on_711-2N_333.4dfp.img'];
    tmprifh{i,1} = [ QC(i).subatlasdir '/' QC(i).vcnum '_mpr_on_711-2N_333.4dfp.ifh'];
    ...
    for j=1:numel(QC(i).restruns)
        ...
        tboldmat{i,j} = [ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d.mat'];
        tboldimg{i,j} = [ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d_atl.4dfp.img'];
        tboldifh{i,j} = [ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d_atl.4dfp.ifh'];
...
for i=1:numsubs
    for j=1:numel(QC(i).restruns)
        LASTIMG{i,j,stage}=[ QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d_atl'];
    end
    cd(QC(i).subdir);
...
for i=1:numsubs
    for j=1:numel(QC(i).restruns)
        ...
        rdatfile{i,j}= [QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d.rdat'];
        ddatfile{i,j}= [QC(i).subbolddir{j} '/' QC(i).vcnum '_b' num2str(QC(i).restruns(j)) '_rmsp_faln_dbnd_xr3d.ddat'];
...
%     end
    ...
    if switches.doblur
        ...
        % % % checking for NaNs in file before running gauss_4dfp to prevent data being blacked out.
        pre_file_name = textread([LASTCONC{stage-1} '.conc'], '%s', 'headerlines', 1);
        pre_file_name = pre_file_name{1}(6:end);
        pre_file = read_4dfpimg(pre_file_name);
        pre_file(find(isnan(pre_file))) = 0;
        write_4dfpimg(pre_file, pre_file_name, 'bigendian')
        clear pre_file pre_file_name
...
% cd(tdir);
% write a corrfile
fid=fopen([tdir, '/QC/', df.vcnum{1}, '_corrfile.txt'], 'w');
for i=1:numsubs
    fprintf(fid,'%s\t%s\t%s\n',[QC(i).subdir '/' QC(i).vcnum '_' allends '.4dfp.img'],[QC(i).subdir '/total_tmask.txt'],QC(i).vcnum);
end
fclose(fid);
save([tdir, '/QC/', df.vcnum{1}, '_QC.mat'], 'QC','-v7.3');
cd(startdir);

    catch err
        err_count = err_count + 1;
        failed_subjects{err_count, 1} = df_all.vcnum(count);
        failed_subjects_count = [failed_subjects_count; count];
        failed_subjects_err = [failed_subjects_err; err];
        save([startdir '/error_log.mat'], 'failed_subjects', 'failed_subjects_count', 'failed_subjects_err', 'df_all', '-v7.3')
        rethrow(err) % hopefully throws/shows the exact error that occured
    end
