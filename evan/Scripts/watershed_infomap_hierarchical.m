...
dlmwrite(prmfilename,'parcel_hierarchical','-append','delimiter','');
...
%% Run infomap
cd(outputfolder)
graphcluster_Evan_surfacexd_v157([outputfolder '/prmfile.txt'],'thr',distanceexclusion,xdistancemat,0,0,'infomap');

%% Modify color assignments with miminum network size criterion

filesinoutputfolder = dir(outputfolder);
folderdatecreated = zeros(length(filesinoutputfolder)-2,1);
for i=3:length(filesinoutputfolder);
    if logical(filesinoutputfolder(i).isdir)
...
simple_assigns = modify_clrfile('simplify','rawassn_hier.txt',networksizeminimum);

%% Assign communities to watersheds, LR together
clear waternum

simple_assigns = load(['rawassn_hier_minsize' num2str(networksizeminimum) '.txt']);
...
%  tallymat_infomap(assns(:,8:end),[outputfolder '/' roifilename]);
