function correls = parcel_homogeneity_PCA_cov_testing(multiwatershed,corr_LR,iscifti,hem)
...
%     end
        ...
        %watercovcorr = corr_LR(brainindices{parcel},brainindices{parcel});
        
        
%        if size(watercovcorr,1) > 2
            
            correlpatterns = corr_LR(:,brainindices{parcel} + (29696*(strcmp(hem,'R'))));
            
            [Y V_use eigvals_per] = PCA_reduction(correlpatterns,'comps',2);
            correls(parcel) = corr(Y(2,:)',mean(correlpatterns,2));
%         else
