...
%     ciftifiles{s} = ['/data/hcp-zfs/shared-nil/adeyemob/HCP_MAIN/OTHER/CIFTI/' subjects{s} '/' subjects{s} '_concat_222_LR_surf_subcort_normalwall.dtseries.nii'];
% end
...
for s = 1:length(subjects)
    ...
    for templatenum = 1:size(ThreshTemplates,2);
        string = ['Subject ' num2str(subnums(s)) ': comparing to network ' num2str(IDs(templatenum))];
        fprintf([repmat('\b',1,length(prevstring)) '%s'],string);
        prevstring = string;
