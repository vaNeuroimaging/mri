function outputcifti = metric_cluster_cifti_surfacearea(cifti,minthresh,maxthresh,minsize,surfaceareafiles)
%outputcifti = metric_cluster_cifti_surfacearea(cifti,minthresh,maxthresh,minsize,surfaceareafiles)
...
for hemnum = 1:2
    ...
    metric = zeros(size(medial_wall{hemnum}));
    metric(medial_wall{hemnum}==0) = cifti(ciftiinds{hemnum});
    
    
    if exist('surfaceareafiles') && (~isempty(surfaceareafiles))
        surfacearea = gifti(surfaceareafiles{hemnum});
    else
    ...
    for clusternum = 1:length(uniqueclustervals)
        ...
        if sum(surfacearea(clusteredmetric==uniqueclustervals(clusternum))) > minsize
    ...
    if any(outputmetric)
        outputcifti(ciftiinds{hemnum}, (end+1) : (end+size(outputmetric,2))) = outputmetric(medial_wall{hemnum}==0,:);
    end
