...
resultsfolder = '/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_60/CorrectRot/';
...
Rankthreshs = [500 550 600 650 700 750 800 850 900];
Homothreshs = [.6 .65 .7 .75 .8 .85 .9];
...
for i = 1:length(Homothreshs)
    for j = 1:length(Rankthreshs)
        Pctabovethreshs(i,j,parcelset) = nnz((realeigvals_per_first > Homothreshs(i)) .* (Ranks > Rankthreshs(j))) / length(Ranks);
    end
end
