function  surface_parcellation_lowRAM(cohortfile,tmasklist,subsample,dosubcort,outputdir)
...
%         save([outputdir '/randinds_used.mat'],'randinds')            
    ...
    subsampled_corrmap = single(FisherTransform(subsampled_corrmap));
    ...
    corrofcorr = zeros(size(fullgrads),'single');
    
    divisionsize = ceil(size(fullgrads,1)/divisions);
    disp('Calculating similarity map')
    for division = 1:length(divisions)
        divisioninds = [((division-1) * divisionsize +1) : min([size(fullgrads,1), (division * divisionsize)])];
        ...
        division_corrmap = single(FisherTransform(division_corrmap));
