function [groupmatches,submatches,groupclus_dmat] = networkClusters_match_preclustered(groupclustermap,groupclusternetworkIDs, distances, groupclus_dmat,subclustermap,subclusternetworkIDs,distthresh)
%[groupmatches,submatches,groupclus_dmat] = networkClusters_match_preclustered(groupclustermap,groupclusternetworkIDs, distances, groupclus_dmat,subclustermap,subclusternetworkIDs,distthresh)
...
if isempty(groupclus_dmat)
    groupclus_dmat = cell(length(groupIDs),1);
    for groupIDnum = 1:length(groupIDs)
        ...
        groupclus_dmat{groupIDnum} = cell(size(groupclusters,2),1);
        for i = 1:size(groupclusters,2)
            groupclusters(groupclustermap==groupclusternums_thisID(i),i) = 1;
            groupclus_dmat{groupIDnum}{i} = distances(logical(groupclusters(:,i)),:);
        end
    end
end
