%function  surface_parcellation_indsubs(cohortfile,outputdir,tmasktype,smooth,hems,edges)
...
cohortfile = '/data/cn4/evan/RestingState/FC_Mapping_120/NonstationarityParcellation/LFRS_corrfile2.txt';
surfdatadir = '/data/cn4/evan/RestingState/FC_Mapping_120/NonstationarityParcellation/Toptimes/';
outputdir = '/data/cn4/evan/RestingState/FC_Mapping_120/NonstationarityParcellation/Toptimes/';
tmasktype = '/data/cn4/evan/RestingState/FC_Mapping_120/NonstationarityParcellation/LFRS_tmasklist2.txt';
smooth = 'yes';
hems = 'LEFT';
edges = 'yes';
...
%                system(['rm ' BOLDname '.func.gii'])
        ...
        thresholdtime = floor(threshold*size(surf_BOLD,2));
        
        [ign highsortedindex] = sort(surf_BOLD,2,'descend');
        [ign lowsortedindex] = sort(surf_BOLD,2,'ascend');
        
        threshsortedindex = [highsortedindex(:,1:thresholdtime) lowsortedindex(:,1:thresholdtime)];
        
        crosscorr = zeros(size(surf_BOLD,1),size(vol_BOLD,2));
        
        % Calculate correlation between surface and volume data
        fprintf('Calculating correlation between surface nodes and volume...')
        
        for vert = 1:size(surf_BOLD,1)
        
        %crosscorr = FisherTransform(paircorr_mod(single(surf_BOLD)',single(vol_BOLD)));
        
            crosscorr(vert,:) = paircorr_mod(single(surf_BOLD(vert,threshsortedindex(vert,:)))',single(vol_BOLD(threshsortedindex(vert,:),:)));
            
        end
        
        crosscorr = FisherTransform(crosscorr);
        ...
        filestem = [subject '_avgcorrofcorr_smooth' num2str(smoothnum) '_allgrad_' HEMS{hem} '_toptimes'];
