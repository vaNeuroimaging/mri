function out = Colormatch_networkIDs(regularizedfilename,distances)
...
%     end
    ...
    stillunassignedIDs = allsubIDs{col};
...
for col = 1:size(subnetworks,2)
    ...
    string{col} = ['Assigning cleaning maps for column ' num2str(col) ' of ' num2str(size(subnetworks,2))];
    if col==1; 
        fprintf('%s',string{col}); 
    elseif col==size(subnetworks,2); 
        fprintf([repmat('\b',1,length(string{col-1})) '%s'],string{col}); 
    ...
    for assignednum = 1:length(assign)
        if (assign(assignednum)) > 0 %&& (all_mean_mindists{col}(assignednum,assign(assignednum)) < costcutoff)
            ...
            stillunassignedIDs(stillunassignedIDs==allsubIDs{col}(assign(assignednum))) = [];
        end
    end
...
%     end
    ...
    for extraIDnum = 1:length(stillunassignedIDs)
        matched(filledsubnetworks(:,col)==stillunassignedIDs(extraIDnum)) = extraassignments(extraIDnum);
    end
...
out = zeros(size(groupnetworks,1),size(allmatched,2));
out(1:nsurfverts,:) = allmatched;
outfilename = [regularizednetworksfolder '/Recolored_Powercolors_cleaned_' regularizednetworksfile];
