...
watershednames = {'Poldrome_subsurf_edge_L_watershedmerge_0.45.func.gii','Poldrome_subsurf_edge_R_watershedmerge_0.45.func.gii'};
...
outputfolder = [waterdir 'Poldrome_subsurf_edge_LR_watershedmerge_045_infomap/'];
...
outputstem = 'Poldrome_subsurf_edge_LR_watershedmerge_045_watershedassignments';



boostrap_iterations = 1;%500;
...
% %         evalc(['!awk ''NF > 25'' ' waterdir '/Temp2.func.gii > ' waterdir '/Temp2_noHEAD.func.gii']);
        ...
        cifti_timeseries = cifti_read(['/data/hcp-zfs/home/laumannt/Poldrome/shared_for_washu/FCPROCESS_SCRUBBED_meanfield_lininterp_222_sub018_reg_FD025/cifti_timeseries_normalwall/' subjects{s} '_BOLD_LR_surf_subcort_333_32k_fsLR_smooth2.55.dtseries.nii']);
...
%% Run infomap
cd(outputfolder)
graphcluster_Evan_surfacexd_confidence([outputfolder '/prmfile.txt'],'thr',distanceexclusion,xdistancemat,0,'infomap',std_water_corrmat,boostrap_iterations);
...
simple_assigns = modify_clrfile('simplify','rawassn.txt',networksizeminimum);
 ...
 confidence = load('confidence.txt');
confidence(simple_assigns<1) = 0;
dlmwrite('confidence_minsize3.txt',confidence,' ')

second_id = load('second_id.txt');
second_id(simple_assigns<1) = -1;
%second_id(confidence>(1-alpha)) = -1;
dlmwrite('second_id_minsize3.txt',second_id,' ')
%% Assign communities to watersheds, LR together
clear waternum

% if length(HEMS) > 1
%      ciftitemplatefile = ['/data/cn4/evan/RestingState/Consensus/ConsensusMapvFinal_ciftitemplate.func.gii'];
...
%     if length(HEMS) > 1
%         ciftidata_towrite((1:nnz(mask==0)) + (nnz(medial_wall{1})*(hem-1)) , t) = water_com_assigns(mask==0);
%     end
...
for t = 1:size(simple_assigns,2)
    ...
    for i = 1:length(waternum{hem})
        ...
        water_com_assigns(watershed==waternum{hem}(i),t) = confidence(i + (length(waternum{1})*(hem==2)),t);
    end
%     if length(HEMS) > 1
%         ciftidata_towrite((1:nnz(mask==0)) + (nnz(medial_wall{1})*(hem-1)) , t) = water_com_assigns(mask==0);
%     end
...
save(gifti(single(water_com_assigns)),[outputstem HEMS{hem} '_minsize' num2str(networksizeminimum) '_confidence.func.gii']);
...
for t = 1:size(simple_assigns,2)
    ...
    for i = 1:length(waternum{hem})
        ...
        water_com_assigns(watershed==waternum{hem}(i),t) = second_id(i + (length(waternum{1})*(hem==2)),t);
    end
%     if length(HEMS) > 1
%         ciftidata_towrite((1:nnz(mask==0)) + (nnz(medial_wall{1})*(hem-1)) , t) = water_com_assigns(mask==0);
%     end
...
save(gifti(single(water_com_assigns)),[outputstem HEMS{hem} '_minsize' num2str(networksizeminimum) '_second_id.func.gii']);

end
      
%  if length(HEMS) > 1
%      cifti_write_wHDR(ciftidata_towrite,ciftitemplatefile,[outputstem '_LR_minsize' num2str(networksizeminimum)])
%  end
