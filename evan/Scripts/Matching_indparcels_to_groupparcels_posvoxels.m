...
outputfolder = '/data/cn4/evan/RestingState/Ind_variability/Subjects/Assignments/Matching_expand/'; %location match data will be written to
...
subdistancemap = zeros(nnz(mask),1);
    ...
    for subparcelnum = 1:length(subparcelIDs)
        ...
        for parcelnum = 1:length(parcelIDs)
            ...
            indicestocorrelate = logical(ones(length(subparcelcorrelpattern),1)); 
            %posneginds = union(find(parcelcorrelpatterns(parcelnum,:)>.15),find(parcelcorrelpatterns(parcelnum,:)<-.15));
            %indicestocorrelate(posneginds) = 1;
            indicestocorrelate(indicestoavoid) = 0;
...
% end
    ...
    disp([num2str(matchsuccess) ' of ' num2str(length(parcelIDs)) ' group parcels successfully matched'])
...
save(gifti(single(groupassignoutput)),[outputfolder 'Group_parcels_matchtosubs4_' hem '.func.gii'])

save(gifti(single(groupsimilarityoutput)),[outputfolder 'Group_parcels_matchtosubs4_similarity_' hem '.func.gii'])

%save(gifti(single(groupdistanceoutput)),[outputfolder 'Group_parcels_assignedtosubs_distance_' hem '.func.gii'])

save(gifti(single(subassignoutput)),[outputfolder 'Subject_parcels_matchtogroup4_' hem '.func.gii'])

save(gifti(single(subsimilarityoutput)),[outputfolder 'Subject_parcels_matchtogroup4_similarity_' hem '.func.gii'])
...
save(gifti(single(parcelexistancemap)),[outputfolder 'Group_parcels_matchnumber4_' hem '.func.gii']);
save(gifti(single(parceloverlapmap)),[outputfolder 'Sub_parcels_overlap4_' hem '.func.gii']);
