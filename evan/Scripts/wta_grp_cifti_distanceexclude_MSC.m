function wta_grp_cifti_distanceexclude_MSC(MSCname,cortical_map,regionname,corr_type,sign,xdistance)
...
fprintf(log,'wta_grp.m log\n corrfile = %s\n cortical map = %s\n region = %s\n correlation type = %s\n sign = %s\n distance exclusion = %s\n', ...
    MSCname,cortical_map,regionname,corr_type,sign,num2str(xdistance));
fclose(log);
...
cort_datamat_cifti = ft_read_cifti_mod(cortical_map); cort_datamat = cort_datamat_cifti.data;
...
braininds_withdata = cort_datamat_cifti.brainstructure; 
braininds_withdata = braininds_withdata(braininds_withdata>0);
subcort_datamat = braininds_withdata > 2;
clear cort_datamat_cifti
...
for s = 1:numel(subjectlist)
    ...
    L_surf_coordfile = ['/data/nil-bluearc/GMT/Laumann/MSC/fs5.3_native/FREESURFER_fs_LR/' MSCname '/NativeVol_fs_LR/fsaverage_LR32k/' MSCname '.L.midthickness.32k_fs_LR.coord.gii'];
    L_surf_coords = gifti(L_surf_coordfile); L_surf_coords = L_surf_coords.vertices;
    R_surf_coordfile = ['/data/nil-bluearc/GMT/Laumann/MSC/fs5.3_native/FREESURFER_fs_LR/' MSCname '/NativeVol_fs_LR/fsaverage_LR32k/' MSCname '.R.midthickness.32k_fs_LR.coord.gii'];
    R_surf_coords = gifti(R_surf_coordfile); R_surf_coords = R_surf_coords.vertices;
    LR_surf_coords = [L_surf_coords ; R_surf_coords];
    LR_surf_coords = LR_surf_coords(cifti_template.brainstructure(1:length(LR_surf_coords)) > 0,:);
    
    subcort_coords = cifti_template.pos(length(L_surf_coords)+length(R_surf_coords)+1:end,:);
    
    cort_to_subcort_dists = pdist2(LR_surf_coords,subcort_coords);
    
    corrmat = zeros(length(subcort_ind),size(cort_ind,1));
    tot_corrmat = zeros(length(subcort_ind),size(cort_ind,1));
    
    for indnum = 1:length(subcort_ind);
        ind_timecourse = cifti_datamasked(subcort_ind(indnum),:);
        valid_cort_inds = cort_to_subcort_dists(:,indnum) > xdistance; valid_cort_inds((end+1):size(cort_datamat,1)) = 0;
        cort_timcourses = zeros(size(cort_ind,1),size(cifti_datamasked,2));
        for b = 1:size(cort_ind,1)
            valid_system_inds = (valid_cort_inds & (cort_datamat == cort_ind(b)));
            cort_timcourses(b,:) = mean(cifti_datamasked(valid_system_inds,:),1);
        end
        
        if strcmp(corr_type,'part')
            for b = 1:size(cort_ind,1)
                subset_cort_timcourses = cort_timcourses;
                subset_cort_timcourses(b,:) = [];
                corrmat(indnum,b) = partialcorr(ind_timecourse',cort_timcourses(b,:)',subset_cort_timcourses');
            end
        end
        tot_corrmat(indnum,:) = corr(ind_timecourse',cort_timcourses');
    end
    if strcmp(corr_type,'tot')
        corrmat = tot_corrmat;
    end
