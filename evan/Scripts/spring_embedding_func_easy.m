function [h nodepos] = spring_embedding_func_easy(corrmat,assignments,kden,Kc,L0)
% Spring-embedding algorithm
%[h nodepos] = spring_embedding_func_easy(corrmat,assignments,kden,Kc,L0)
%Kc: try 1
%L0:try 25
...
%[graph, r, kden] = matrix_thresholder(corrmat,kden,'kden');

corrmat_vec = corrmat(triu(true(size(corrmat)),1));
sorted = sort(corrmat_vec,'descend');
r = sorted(ceil(length(sorted) .* kden));
graph = corrmat > r;
...
disp(' ')
    ...
    int=1;
    for i = 1:nodes
            plot(nodepos(int,1),nodepos(int,2),'MarkerFaceColor',[colors(i,1) colors(i,2) colors(i,3)],'Marker','o','MarkerSize',12,'MarkerEdgeColor','k','LineWidth',2);
            int = int+1;
    end
%     
%     % For module coloring
