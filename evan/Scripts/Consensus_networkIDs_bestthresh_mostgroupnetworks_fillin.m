function Consensus_networkIDs_bestthresh_mostgroupnetworks_fillin(regularizedfilename,distances)
...
for col = 1:size(subnetworks,2)
    ...
    for groupIDnum = 1:length(groupIDs)
        ...
        for subIDnum = 1:length(subIDs)
            ...
            noverlapverts = nnz((surfgroupnetworks==groupID) .* (surfsubnetworks==subID));
...
disp(['Highest column with most networks matched is column ' num2str(bestmatch_column)])

%Map the minimum cost color assignments onto the brain
assign = colassigns{bestmatch_column};
unassignedIDs = allsubIDs{bestmatch_column};

for assignednum = 1:length(assign)
    if (assign(assignednum)) > 0 && (all_mean_mindists{bestmatch_column}(assignednum,assign(assignednum)) < costcutoff)
        matched(filledsubnetworks(:,bestmatch_column)==allsubIDs{bestmatch_column}(assign(assignednum))) = groupIDs(assignednum);
        unassignedIDs(unassignedIDs==allsubIDs{bestmatch_column}(assign(assignednum))) = [];
    end
end
...
for extraIDnum = 1:length(unassignedIDs)
    vertstofillin = [vertstofillin; find(filledsubnetworks(:,bestmatch_column)==unassignedIDs(extraIDnum))];
end

subIDsassigned = zeros(length(assign),1);
for i = 1:length(assign)
    if assign(i) > 0
        subIDsassigned(i) = allsubIDs{bestmatch_column}(assign(i));
    end
end
...
for vert = vertstofillin'
    for col = bestmatch_column+1:size(filledsubnetworks,2)
        if any(subnetworks(vert,col)==allsubIDs{bestmatch_column}(assign_nozeros))
            matched(vert) = groupIDs(subIDsassigned==subnetworks(vert,col));
            break
        end
        if col==size(filledsubnetworks,2)
            stillunassignedIDs = unique([stillunassignedIDs filledsubnetworks(vert,bestmatch_column)]);
...
for extraIDnum = 1:length(stillunassignedIDs)
    recoloredverts(filledsubnetworks(:,bestmatch_column)==stillunassignedIDs(extraIDnum)) = extraassignments(extraIDnum);
end
