...
% pallete. This is only permitted if the values in the cifti are integers
% between 1 and 18.
%
%E. Gordon 07/10/16
...
if ischar(bordercolors) && strcmp(bordercolors,'default') 
    if any(IDs < 1) || any(IDs > 18) || any(mod(IDs,1))
        disp('Warning: Use of (default) power colors requires values to be integers between 1 and 18. Borders will not be drawn around non-compliant values.')
        data_bothhems(:,IDs<1) = [];
        IDs(IDs<1) = [];
        data_bothhems(:,IDs>18) = [];
        IDs(IDs>18) = [];
        data_bothhems(:,logical(mod(IDs,1))) = [];
        IDs(logical(mod(IDs,1))) = [];
    end
