function systemresult = post_fc_processing_batch_singlesub_nosmooth(subject,voldata_tomap,outfolder,fsLRfolder,TR,sequence_name,tmaskfile,preproc_datalist,runs_sessionsfile,medial_masks,systemresult)
...
for hem = 1:2
    ...
    surfname_final{hem} = [surffuncdir '/' surfname '_dil10_32k_fs_LR.func.gii'];
...
[systemresult{end+1,1},systemresult{end+1,2}] = system([workbenchdir '/wb_command -cifti-create-dense-timeseries ' ciftidir '/' sequence_name '_LR_surf_subcort_333_32k_fsLR.dtseries.nii -volume ' funcvol '.nii.gz ' subcort_mask ' -left-metric ' surfname_final{1} ' -roi-left ' medial_mask_L ' -right-metric ' surfname_final{2} ' -roi-right ' medial_mask_R ' -timestep ' num2str(TR) ' -timestart 0']);

if do_smallwall
    [systemresult{end+1,1},systemresult{end+1,2}] = system([workbenchdir '/wb_command -cifti-create-dense-timeseries ' ciftiswdir '/' sequence_name '_LR_surf_subcort_333_32k_fsLR.dtseries.nii -volume ' funcvol '.nii.gz ' subcort_mask ' -left-metric ' surfname_final{1} ' -roi-left ' sw_medial_mask_L ' -right-metric ' surfname_final{2} ' -roi-right ' sw_medial_mask_R ' -timestep ' num2str(TR) ' -timestart 0']);
end
