function Consensus_networkIDs_bestthresh_matchtogroup_fillin(regularizedfilename,distances)
...
for col = 8%1:size(subnetworks,2)
    ...
    cost(col) = sum(assignedcosts); %cutoff*(nnz(unique(subnetworks(:,col)))-nnz(colassigns{col}));
...
for extraIDnum = 1:length(unassignedIDs)
    vertstofillin = [vertstofillin; find(filledsubnetworks(:,mincol)==unassignedIDs(extraIDnum))];
end

subIDsassigned = zeros(length(assign),1);
for i = 1:length(assign)
    if assign(i) > 0
        subIDsassigned(i) = allsubIDs{mincol}(assign(i));
    end
end
...
for vert = vertstofillin'
    for col = mincol+1:size(filledsubnetworks,2)
        if any(subnetworks(vert,col)==allsubIDs{mincol}(assign_nozeros))
            matched(vert) = groupIDs(subIDsassigned==subnetworks(vert,col));
            break
        end
        if col==size(filledsubnetworks,2)
            stillunassignedIDs = unique([stillunassignedIDs filledsubnetworks(vert,mincol)]);
        end
    end
end
...
for extraIDnum = 1:length(stillunassignedIDs)
    matched(filledsubnetworks(:,mincol)==stillunassignedIDs(extraIDnum)) = extraassignments(extraIDnum);
end
...
outfilename = [regularizednetworksfolder '/Consensusfillin_Powercolors_cleaned_' regularizednetworksfile];
cifti_write_wHDR(out,[],outfilename)
