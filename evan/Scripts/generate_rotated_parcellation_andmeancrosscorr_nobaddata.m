function generate_rotated_parcellation_andmeancrosscorr_nobaddata(watershedname,iterations,crosscorr,rotations,iscifti,hem,outfileprefix)
...
parcelIDs = unique(realwatershed); parcelIDs(parcelIDs==0) = [];
meancrosscorrvals = zeros(size(realwatershed));
for parcelnum = 1:length(parcelIDs)
    parcelindices = find(realwatershed==parcelIDs(parcelnum));
    trueparcelindices = cross_datatype_indices(parcelindices,iscifti+1); trueparcelindices = trueparcelindices(logical(trueparcelindices));
    parcel_crosscorr = crosscorr(trueparcelindices,trueparcelindices);
    mean_crosscorr = mean(parcel_crosscorr(logical(triu(ones(size(parcel_crosscorr)),1))),1);
    meancrosscorrvals(parcelindices) = mean_crosscorr;
end
...
save(gifti(single(meancrosscorrvals)),['Meancrosscorr_' watershedname(watershedslashloc(end)+1:end)])
...
%         end
        ...
        if (numel(intersect(rotparcelindices,find(mask))) == 0)  && ((numel(borderverts) / numel(rotparcelindices)) < 3) && (numel(intersect(rotparcelindices,find(~gooddata))) < 15)
            ...
            truerotparcelindices = cross_datatype_indices(rotparcelindices,iscifti+1); truerotparcelindices = truerotparcelindices(logical(truerotparcelindices));
            
            parcel_crosscorr = crosscorr(truerotparcelindices,truerotparcelindices);
            mean_crosscorr = mean(parcel_crosscorr(logical(triu(ones(size(parcel_crosscorr)),1))),1);
            rotated_eigvals(parcelnum,iternum) = mean_crosscorr;
...
for watershednum = 1:length(watersheds)
    ...
    realeigvals_per_first(watershednum) = mean(meancrosscorrvals(templabel==watershednum));
