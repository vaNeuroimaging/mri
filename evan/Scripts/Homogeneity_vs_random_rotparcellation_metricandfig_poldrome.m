...
%allparcelfilenames = {'/data/cn4/evan/RestingState/FC_Mapping_120/subsurf/nosmooth/120_subsurf_L_test_watershedmerge_0.6.func.gii','/data/cn4/evan/RestingState/FC_Mapping_120/subsurf/nosmooth/120_subsurf_R_test_watershedmerge_0.6.func.gii'};
...
%'/data/cn4/evan/RestingState/FC_Mapping_120/Random_matchedto_120_L_wateredgethresh_watershedmerge_0.45.func.gii','/data/cn4/evan/RestingState/FC_Mapping_120/Random_matchedto_120_R_wateredgethresh2_watershedmerge_0.45.func.gii'};
...
%     '/data/cn4/evan/ROIs/264_surfvert_ROIs_L_removeedge.func.gii','/data/cn4/evan/ROIs/264_surfvert_ROIs_R_removeedge.func.gii';...
...
names = {'Boundary map','Power ROIs','Craddock','Shen','Power Communities','Yeo','Brodmann','AAL'};


%resultsfolder = '/data/cn4/evan/RestingState/FC_Mapping_120/AlternateHomogeneity_testing_in_108/ReplaceNan/';
%resultsfolder = '/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_108/ReplaceNan/';
%resultsfolder = '/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_120/RotParcellation/ReplaceNan/';
...
%colors = {'r','g','c','m','b','y'};
colors = {[1 0 0],[0 1 0],[.3 0 .6],[.2 1 1],[1 .5 0],[0 0 1],[1 1 0],[1 .6 1]};
...
%         allbadvertices = find(logical(mask + (~gooddata)));
        ...
        baddataname = ['/data/cn4/evan/RestingState/Ind_variability/Poldrome/Poldrome_baddata_bigcluster_' hem '.func.gii'];
        baddata = gifti(baddataname); baddata = baddata.cdata;
        gooddata = (mask==0) .* (~baddata);
...
%     end
    ...
    meanwindowsizes = [];
    meanwindowZscores = [];
    for i = 1:(length(sorti)-runningwindow+1)
        window = sorti(i:(i+runningwindow-1));
        meanwindowsizes(i) = mean(realsizes(window));
        meanwindowZscores(i) = mean(Zscores(window));
    end
    
    randadd = ((rand(size(meanwindowsizes))*2) -1) * .001;
    datain = [(meanwindowsizes+randadd)' , meanwindowZscores'];
    evalc('[dataout lowerLimit upperLimit xy] = lowess(datain,f,0);');
    fittedwindowZscores{parcelset} = xy(:,2);
    windowsizes{parcelset} = meanwindowsizes;
...
% % end
% 
% plot_reorder = [8 7 6 5 4 3 2 1];
% 
% nullhandle = plot(allsortedsizes,allrotfittedvals,'k-','LineWidth',4);
% plot(allsortedsizes,allrotfittedvals,'k-','LineWidth',7);
% linehandles = [];
% for parcelset = plot_reorder%1:size(allparcelfilenames,1)
%     plot(sortedsizes{parcelset},realfittedvals{parcelset},'k-','LineWidth',7)
%     thishandle = plot(sortedsizes{parcelset},realfittedvals{parcelset},'-','LineWidth',4,'Color',colors{parcelset});
%     linehandles = [linehandles thishandle];
% end
% legend([linehandles(plot_reorder) nullhandle],[names {'Null model'}])
% set(gcf,'Color',[1 1 1])
% xlim([0 3100])
% set(gca,'FontSize',20)
...
% hold on
% for parcelset = 1:size(allparcelfilenames,1)
%     plot(windowsizes{parcelset},fittedwindowZscores{parcelset},'k-','LineWidth',7)
%     %plot(sortedsizes{parcelset},realfittedvals{parcelset},[colors{parcelset} '-'],'LineWidth',4)
%     plot(windowsizes{parcelset},fittedwindowZscores{parcelset},'-','LineWidth',4,'Color',colors{parcelset})
% end
% set(gcf,'Color',[1 1 1])
% set(gca,'FontSize',30)
