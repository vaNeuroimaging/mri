...
clustersizemapname = 'Subject_clustering_vertexwise_allnetworks_L_minQ_0.1_clustersize.func.gii';
clustersizemap = gifti(clustersizemapname); clustersizemap=clustersizemap.cdata;
...
for networkID = networkIDs'
    ...
    for clusternum = uniqueclustervals'
        if nnz(clusteredmetric==clusternum) < minvertcluster
...
sizemap = zeros(size(variabilitymap,1),length(networkIDs));
for IDnum = 1:length(networkIDs)
    ...
    sizemap(networkindices,IDnum) = -1;
    
    outside_network_indices = find(sum((variabilitymap(:,2:end)==networkID),2));
    
    for vertex = outside_network_indices'
        
        colnum = find(variabilitymap(vertex,:)==networkID);
        size = sum(clustersizemap(vertex,colnum));
        sizemap(vertex,IDnum) = size;
        
    end
end

save(gifti(single(sizemap)),[variabilitymapname(1:end-9) '_clustersize_bynetwork.func.gii'])
