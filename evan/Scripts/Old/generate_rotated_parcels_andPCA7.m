function generate_rotated_parcels_andPCA7(watershedname,iterations,cov_corr,iscifti,hem,outfileprefix)
%generate_rotated_parcels_andPCA7(watershedname,iterations,avgcrosscorrname,iscifti,hem,outfileprefix)
...
%     end
    ...
    for iternum = 1:iterations
        string{iternum} = ['Parcel ' num2str(parcelnum) ': rotation ' num2str(iternum)];
        if iternum==1; fprintf('%s',string{iternum}); else fprintf([repmat('\b',1,length(string{iternum-1})) '%s'],string{iternum}); end
        ...
        while done == 0
            ...
            indexcoords = spherecoords(centerind,:)';
            xrotcoords = rotmat_x * indexcoords;
            xyrotcoords = rotmat_y * xrotcoords;
            xyzrotcoords = rotmat_z * xyrotcoords;
            
            mattotest(:,1) = xyzrotcoords(1);
            mattotest(:,2) = xyzrotcoords(2);
            mattotest(:,3) = xyzrotcoords(3);
            %repmat(xyzrotcoords',[32492 1]);
            diff_test = sum(abs(spherecoords - mattotest),2);
            [val targetind] = min(diff_test);
            
            if nnz(geo_distances(targetind,allbadvertices)<maxdist) == 0
                done = 1;
            end
        end
        ...
        for n = 1:length(realparcelindices)
            mattotest(:,1) = xyzrotcoords(1,n);
            mattotest(:,2) = xyzrotcoords(2,n);
            mattotest(:,3) = xyzrotcoords(3,n);
            %mattotest = repmat(xyzrotcoords(:,n)',[32492 1]);
            diff_test = sum(abs(spherecoords - mattotest),2);
    ...
    fprintf(repmat('\b',1,length(string{iternum})))
    %    end
