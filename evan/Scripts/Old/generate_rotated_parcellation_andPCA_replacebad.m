function generate_rotated_parcellation_andPCA_replacebad(watershedname,iterations,cov_corr,rotations,iscifti,hem,outfileprefix)
...
baddata1 = gifti(['/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_60/RotParcellation/Baddata_1_' hem '.func.gii']); baddata1 = baddata1.cdata;
baddata2 = gifti(['/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_60/RotParcellation/Baddata_2_' hem '.func.gii']); baddata2 = baddata2.cdata;

replace_baddata1 = gifti(['/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_60/RotParcellation/RotatedBaddata_1_' hem '.func.gii']); replace_baddata1 = replace_baddata1.cdata;
replace_baddata2 = gifti(['/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_60/RotParcellation/RotatedBaddata_2_' hem '.func.gii']); replace_baddata2 = replace_baddata2.cdata;
replace_medwall = gifti(['/data/cn4/evan/RestingState/FC_Mapping_120/Homogeneity_testing_in_60/RotParcellation/RotatedMedialWall_' hem '.func.gii']); replace_medwall = replace_medwall.cdata;

cov_corr_temp = zeros(32492);
cov_corr_temp(~logical(mask),~logical(mask)) = cov_corr;
cov_corr = cov_corr_temp; clear cov_corr_temp;
cov_corr(logical(mask),logical(mask)) = cov_corr(logical(replace_medwall),logical(replace_medwall));
cov_corr(logical(baddata1),logical(baddata1)) = cov_corr(logical(replace_baddata1),logical(replace_baddata1));
cov_corr(logical(baddata2),logical(baddata2)) = cov_corr(logical(replace_baddata2),logical(replace_baddata2));
...
for iternum = 1:iterations
    ...
    if isempty(rotations)
        ...
        %phirotation = rand * 2*pi - pi;
        %thetarotation = (rand * pi) - (pi/2);
...
%         thetarotation = theta(targetindex)-theta(centerind);
        ...
        for n = 1:length(realparcelindices)
            %thisindex = realparcelindices(n);
            
            %test = repmat(xyzrotcoords(:,n)',[32492 1]);
        ...
        if ((numel(borderverts) / numel(rotparcelindices)) < 3)
            ...
            watercovcorr = cov_corr(rotparcelindices,rotparcelindices);
