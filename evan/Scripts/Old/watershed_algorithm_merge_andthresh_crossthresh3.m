function watershed_algorithm_merge_andthresh_crossthresh2(edgemetricname,outputdir,filestem,hem,threshs)
...
for wateri = 1:length(watersheds)
    for waterj = 1:length(watersheds)
        ...
        if length(watershedborders{wateri,waterj}) > 1
            ...
            if ((edgeval_withinwater(wateri)*thresh > edgeval_thisborder) && (edgeval_withinwater(waterj)*thresh > edgeval_thisborder)) || (edgeval_thisborder < edgevalmin)
...
label = single(clusteredlabel==0);

label(medialindices) = 0;

multithresh_label(:,threshnum) = label;

end

%%

disp('Combining across thresholds')

acrossthresh_label = mean(multithresh_label,2);


acrossthresh_label_bin = single(acrossthresh_label > parcelborderprob_thresh);
...
while keepgoing == 1
    ...
    verticestocheck = find(acrossthresh_label_bin);
    
    for vertex = verticestocheck'
        vertneighs = neighbors(vertex,2:7);
        numsurrounding = nnz(~isnan(vertneighs));
        vertneighs(isnan(vertneighs)) = [];
        
        if nnz(acrossthresh_label_bin(vertneighs) == 0) > (numsurrounding - 2)
            
            acrossthresh_label_bin(vertex) = 0;
...
acrossthresh_label_bin(medialindices) = 1;
clusteredlabel = zeros(size(acrossthresh_label_bin));

data_inparcels = find(~logical(acrossthresh_label_bin));
...
label(medialindices) = 0;
   %%   
   %save(gifti(single(acrossthresh_label)),[outputdir '/' filestem 'mean_watershedmerge.func.gii'])

save(gifti(label),[outputdir '/' filestem 'watershedmerge.func.gii']);
%save(gifti(multithresh_label),[outputdir '/' filestem 'all_' num2str(threshs(1)) '_to_' num2str(threshs(end)) '_watershedmerge.func.gii']);
