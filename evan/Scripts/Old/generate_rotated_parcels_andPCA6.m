function generate_rotated_parcels_andPCA6(watershedname,iterations,cov_corr,iscifti,hem,outfileprefix)
%generate_rotated_parcels_andPCA6(watershedname,iterations,avgcrosscorrname,iscifti,hem,outfileprefix)
...
for parcelnum = 1:numparcels
    ...
    %find parcel center
    meanX = mean(sphere.vertices(realparcelindices,1));
    meanY = mean(sphere.vertices(realparcelindices,2));
    meanZ = mean(sphere.vertices(realparcelindices,3));
    coord = [meanX meanY meanZ];
    sphere_coords = [sphere.vertices(realparcelindices,1) sphere.vertices(realparcelindices,2) sphere.vertices(realparcelindices,3)];
    ...
    for iternum = 1:iterations
        ...
        for n = 1:length(realparcelindices)
            ...
            theta_new = mod(theta_new_orig(thisparcelindex)+thetarotation,2*pi) - pi/2;
