function watershed_algorithm_merge_edgeval(edgemetricname,outputdir,filestem,hem,avgedgethresh)
%watershed_algorithm_merge5(edgemetricname,outputdir,filestem,hem,thresh)
...
for wateri = 1:length(watersheds)
    for waterj = 1:length(watersheds)
        ...
        if length(watershedborders{wateri,waterj}) > 1
            ...
            if any(edgevals_thisborder < avgedgethresh) %edgeval_thisborder < avgedgethresh
