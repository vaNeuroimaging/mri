function watershed_algorithm_merge_andthresh_targetnumber_noratio(edgemetricname,outputdir,filestem,hem,targetnumber)
...
sortedcorticalmetric = sort(metric(corticalindices),'descend');
...
numberparcels = 0;
thresh = startingthresh;
%while (abs(numberparcels-targetnumber)/targetnumber) > .03
percentoff = 0;
numflips = 0;

while (numberparcels-targetnumber) ~= 0
...
numberparcels = nnz(unique(label .* goodverts));

disp(['Number of parcels:' num2str(numberparcels)])
lastpercentoff = percentoff;
percentoff = (numberparcels-targetnumber)/targetnumber;

if sign(percentoff) ~= -sign(lastpercentoff)

    thresh = thresh + thresh*percentoff;
    %numflips = 0;

else
    numflips = numflips+1;
    thresh = thresh + (thresh*percentoff / (numflips+1));
    if (numflips > 5) && (abs(numberparcels-targetnumber) < 3)
        numberparcels = targetnumber;
    end
...
save(gifti(label),[outputdir '/' filestem 'watershedmerge_noratio.func.gii']);
