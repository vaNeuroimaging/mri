function watershed_algorithm_merge_andZthresh(edgemetricname,outputdir,filestem,hem,thresh)
%watershed_algorithm_merge_andZthresh(edgemetricname,outputdir,filestem,hem,thresh)
...
edgevalthresh = mean(metric(corticalindices));
...
%    edgeval_withinwater(waternum) = mean(edgemetric(label==watersheds(waternum)),1);
     ...
     stdedgeval_withinwater(waternum) = std(valueswithinthiswatershed);
...
for wateri = 1:length(watersheds)
    for waterj = 1:length(watersheds)
        ...
        if length(watershedborders{wateri,waterj}) > 1
            ...
            if (((edgeval_thisborder - edgeval_withinwater(wateri))  / stdedgeval_withinwater(wateri)) < thresh) && (((edgeval_thisborder - edgeval_withinwater(waterj))  / stdedgeval_withinwater(waterj)) < thresh)
