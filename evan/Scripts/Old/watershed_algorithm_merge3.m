function label = watershed_algorithm_merge3(edgemetricname,outputdir,filestem,hem)

% edgemetricname = '/data/cn4/evan/RestingState/FC_Mapping_120/120_edges_L.func.gii';
...
%    edgeval_withinwater(waternum) = mean(edgemetric(label==watersheds(waternum)),1);
     valueswithinthiswatershed = sort(edgemetric(label==watersheds(waternum)));
     edgeval_withinwater(waternum) = mean(valueswithinthiswatershed(1:round(length(valueswithinthiswatershed)*.9)),1);
...
for wateri = 1:length(watersheds)
    for waterj = 1:length(watersheds)
        ...
        if length(watershedborders{wateri,waterj}) > 1
            edgeval_thisborder = mean(edgemetric(watershedborders{wateri,waterj}),1);
...
for waternum = 1:length(watersheds)
    ...
    if (length(find(label==watersheds(waternum))) > 0) && (length(find(label==watersheds(waternum))) <20);
        minedge = 1;
        minwaternum = 0;
