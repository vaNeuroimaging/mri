function wta_grp(corrfile,cortical_map,subcort_map,region,corr_type,sign,varargin)
...
fprintf(log,'wta_grp.m log\n corrfile = %s\n cortical map = %s\n subcortical map = %s\n region = %s\n correlation type = %s\n sign = %s\n regress = %s\n', ...
    corrfile,cortical_map,subcort_map,region,corr_type,sign,reg);
fclose(log);
...
for i = 1:numel(subjectlist)
    ...
    [datamat frames voxelsize] = read_4dfpimg(epi_path);
    epi_datamat = datamat;    
    fidmask = fopen(tmask);
    tmask_array = textscan(fidmask,'%d');
    tmask_col = tmask_array{:,1};
    epi_datamasked = epi_datamat(:,logical(tmask_col)); % epi image with tmask applied
    frame_num = size(epi_datamasked,2);
    ...
    for b = 1:size(cort_ind,1)
        ...
        timecourse = mean(epi_datamasked(ind,:))';   %calculate mean timecourse across all voxels in a cortical region
    ...
    subcort_timecourse = epi_datamasked(subcort_ind,:)';
    
    %%% Regress signal from a mask out of the subcortical region data
    if strcmp(reg_mat,'empty') == 1;
    else
        mask_tc = mean(epi_datamasked(reg_ind,:))';  %create time x mask matrix
...
newname = [num2str(numbersubs) '_fz_' sign '_' corr_type 'corr_' region '_wta_output.4dfp'];
...
newnamet = [num2str(numbersubs) '_t_' sign '_' corr_type 'corr_' region '_wta_output.4dfp'];
write_4dfpimg(newimgt,[newnamet '.img'],'bigendian');
write_4dfpifh([newnamet '.ifh'],1,'bigendian');
system(['ifh2hdr ' newnamet '.img']);

%% Write heat maps for each cortical region and Resample everything to 111 space
...
for p = 1:size(abs_fz_grp_fx,2)
	net_name = ['system' num2str(p) '_fz_' sign '_' corr_type 'corr_' region '_output'];
...
for p = 1:size(tot_abs_fz_grp_fx,2)
	net_name = ['system' num2str(p) '_fz_' sign '_total_corr_' region '_output'];
...
for p = 1:size(ttest_grp,2)
	net_namet = ['system' num2str(p) '_t_' sign '_' corr_type 'corr_' region '_output'];
...
newname111 = [num2str(numbersubs) '_fz_' sign '_' corr_type 'corr_' region '_wta_output_111'];
...
newname111z = [num2str(numbersubs) '_z_' sign '_' corr_type 'corr_' region '_wta_output_111.4dfp'];
...
bg_mask = ['/data/cn4/dgreene/BG_WTA/masks/180_' region '_intersect135_binarymask.4dfp.img'];
