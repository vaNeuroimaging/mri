...
subdistancemap = zeros(nnz(mask),1);
    ...
    groupvsubcost = (1-(groupvsubcorrel+1)/2) ;%+ .5*(1-groupvsub_overlap) + .5*(group_v_sub_distance ./ max_centroiddistance);
    ...
    groupvsubcost(groupvsubcorrel < .1) = Inf;
...
for parcelnum = 1:length(parcelIDs)
    ...
    if (groupassignment(parcelnum) > 0) %&& (onevtwo_pctoverlap(parcelnum,groupassignment(parcelnum)) > 0)
        ...
        if isempty(find(subassignment==parcelnum))
            groupsimilaritymap(parcels==parcelIDs(parcelnum)) = groupvsubcorrel(parcelnum,groupassignment(parcelnum));
            groupdistancemap(parcels==parcelIDs(parcelnum)) = group_v_sub_distance(parcelnum,groupassignment(parcelnum));
        else
            groupsimilaritymap(parcels==parcelIDs(parcelnum)) = mean(groupvsubcorrel(parcelnum,(subassignment==parcelnum)));
...
for parcelnum = 1:length(subparcelIDs)
    ...
    if subassignment(parcelnum) > 0  %&& (onevtwo_pctoverlap(twoassignment(parcelnum),parcelnum) > 0)
        ...
        if isempty(find(groupassignment==parcelnum))
            subsimilaritymap(subparcels==subparcelIDs(parcelnum)) = groupvsubcorrel(subassignment(parcelnum),parcelnum);
            subdistancemap(subparcels==subparcelIDs(parcelnum)) = group_v_sub_distance(subassignment(parcelnum),parcelnum);
        else
            subsimilaritymap(subparcels==subparcelIDs(parcelnum)) = mean(groupvsubcorrel((groupassignment==parcelnum),parcelnum));
...
save(gifti(single(groupassignoutput)),[outputfolder 'Group_parcels_assignedtosubs_' hem '.func.gii'])

save(gifti(single(groupsimilarityoutput)),[outputfolder 'Group_parcels_assignedtosubs_similarity_' hem '.func.gii'])

save(gifti(single(groupdistanceoutput)),[outputfolder 'Group_parcels_assignedtosubs_distance_' hem '.func.gii'])

save(gifti(single(subassignoutput)),[outputfolder 'Subject_parcels_assignedtogroup_' hem '.func.gii'])

save(gifti(single(subsimilarityoutput)),[outputfolder 'Subject_parcels_assignedtogroup_similarity_' hem '.func.gii'])
