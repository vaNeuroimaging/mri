function [groupmatches, submatches] = networkClusters_matchtogroup_temp(subnetworksfilename,distances,groupnetworksfile)
...
for groupIDnum = 1:length(groupIDs)
    ...
    for groupclusnum = 1:size(groupclusters,2)
        for subclusnum = 1:size(subclusters,2)
            ...
            mean_mindists(groupclusnum,subclusnum) = min(mean(submindists),mean([submindists(:); groupmindists(:)]));
