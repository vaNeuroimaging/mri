...
subdistancemap = zeros(nnz(mask),1);
    ...
    for subparcelnum = 1:length(subparcelIDs)
        ...
        if subparcelIDs(subparcelnum)==1767
            1;
        end
        ...
        thisparcel_groupvsubcorrel = groupvsub_voxelcountperc(:,subparcelnum);
...
save(gifti(single(groupassignoutput)),[outputfolder 'Group_parcels_matchtosubscount_' hem '.func.gii'])

save(gifti(single(groupsimilarityoutput)),[outputfolder 'Group_parcels_matchtosubscount_similarity_' hem '.func.gii'])

%save(gifti(single(groupdistanceoutput)),[outputfolder 'Group_parcels_assignedtosubs_distance_' hem '.func.gii'])

save(gifti(single(subassignoutput)),[outputfolder 'Subject_parcels_matchtogroupcount_' hem '.func.gii'])

save(gifti(single(subsimilarityoutput)),[outputfolder 'Subject_parcels_matchtogroupcount_similarity_' hem '.func.gii'])
...
save(gifti(single(parcelexistancemap)),[outputfolder 'Group_parcels_matchnumbercount_' hem '.func.gii']);
save(gifti(single(parceloverlapmap)),[outputfolder 'Sub_parcels_overlapcount_' hem '.func.gii']);
