...
for s = 1:length(subjects)
    ...
    [i r_sub_thresh kden] = matrix_thresholder_faster2(paircorr_mod(data'),kden_thresh,'kden');
    ...
    for ROInum = 1:size(ROIs,2)
        ...
        ROIcorr = ROIcorr .* (ROIcorr >= r_sub_thresh);
...
save(['ROISubVertMat_' num2str(length(trainingnums)) '_kden.mat'],'ROISubVertMat','-v7.3')

[components, eigenvectors, perc_variance_explained] = PCA_reduction_specify_numcomps(ROISubVertMat,2500);

save(['PCA_results_' num2str(length(trainingnums)) '_kden.mat'],'components', 'eigenvectors', 'perc_variance_explained','-v7.3')
...
load(['PCA_results_' num2str(length(trainingnums)) '_kden.mat'])
...
save([num2str(length(trainingnums)) 'train_' num2str(length(MSCnames)) 'test_kden_in.mat'],'training_in','test_in','desired_test','desired_train')


MLP_Training([num2str(length(trainingnums)) 'train_' num2str(length(MSCnames)) 'test_kden_in.mat'])
