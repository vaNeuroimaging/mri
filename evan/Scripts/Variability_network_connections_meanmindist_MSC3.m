...
minclustersizemm = 100;

load /data/cn4/evan/fsaverage_LR32k/Cifti_geo_distances.mat
distances = distances(1:nsurfverts,1:nsurfverts);

distances_totemplate = zeros(size(ThreshTemplates));

for i = 1:size(distances_totemplate,2)
    distances_totemplate(:,i) = min(distances(:,logical(ThreshTemplates(:,i))),2);
end
...
for s = 3%1:length(MSCnames)
    ...
    for i = 1:ncortverts
        ...
        string{i} = ['Vertex ' num2str(i) ' out of ' num2str(ncortverts)];
        if i==1; fprintf('%s',string{i}); else fprintf([repmat('\b',1,length(string{i-1})) '%s'],string{i}); end

        inds = distances(:,i) > xdistance;
        
        Threshvertmap = (correlmaps(:,i) >= r_sub_thresh) .* inds;

        for templatenum = 1:size(templates{templatethresh},2);
            
            ThreshTemplatemap = ThreshTemplates(1:ncortverts,templatenum) .* inds;
            
            thesedistances = distances(logical(Threshvertmap),logical(ThreshTemplatemap));
            
            groupmindists = min(thesedistances,[],1);
            %groupmindists(groupmindists>50) = [];
            
            submindists = min(thesedistances,[],2);
            submindists(submindists > 50) = [];
            
            mean_mindists(templatenum) = mean([submindists(:); groupmindists(:)]);
        end
        
        [mindist, mini] = min(mean_mindists);
        
        thissub_networkconnections(i) = IDs{templatethresh}(mini);
...
cifti_write_wHDR(out,[],['Templatematch_meanmindist_bysubject_' num2str(kden_thresh) 'kden_sub3']);
