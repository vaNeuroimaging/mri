minimadistances = [5];
overlapcostparameters = [0]; %relative weighting of overlap vs connectivity pattern
...
subjBOLDdatafolder = '/data/cn4/evan/RestingState/FC_Mapping_120/WatershedAssignments/';%'/data/cn4/laumannt/fcMapping_redux/surfsmooth_bandpass_surfreg_32k_gradients/';

%makenewgroupstuff = 1;

outputstatsfile = [outputfolder '/temp.txt'];
%[outputfolder '/WatershedAssignments_minimadist5_manyoverlapthresh_comparegroupparcel_localexclusion.txt'];
...
for s = 1:subnum
    ...
    for parcelnum = 1:size(groupwatershed,2)
            ...
            for indparcelnum = 1:size(subjectwatershed,2)
                
                nodestocorrelate = intersect(find(subjectwatershed(:,indparcelnum)==0),find(groupwatershed(:,parcelnum)==0));
                
                subvsgroupcorrel(parcelnum,indparcelnum) = paircorr_mod(avgcorrelmat(:,nodestocorrelate)',subjcorrelmat(:,nodestocorrelate)');
                
            end
        
            watershedmap = FisherTransform(paircorr_mod(mean(surf_BOLD(logical(groupwatershed(:,parcelnum)),:),1)',surf_BOLD(nodestocorrelate,:)'));
            
            watershedmapnomed =  watershedmap; watershedmapnomed(medialmaskind) = 0; watershedmapnomed(logical(isnan(watershedmapnomed))) = 0;
            groupwatershedmapnomed = avgcorrelmat(parcelnum,nodestocorrelate); groupwatershedmapnomed(medialmaskind) = 0;
        
            correlation = paircorr_mod(watershedmapnomed',groupwatershedmapnomed');
        
            texttowrite = [num2str(minimadist) '   Group   Group   ' subjects{s} '   ' num2str(parcelnum) '   Group   ' num2str(correlation) '   1'];
...
for overlapthreshnum = 1:length(overlapthreshvals)
    ...
    save(gifti(single(squeeze(mean(indmatchvalues_ingroupparcels(:,:,overlapthreshnum),2)))),[outputfolder '/AvgSubtoGroupMatchWatershedCorrel_dist' num2str(minimadist) '_overlapthresh' num2str(overlapthresh) '.func.gii']);
    save(gifti(single(squeeze(std(indmatchvalues_ingroupparcels(:,:,overlapthreshnum),[],2)))),[outputfolder '/StdSubtoGroupMatchWatershedCorrel_dist' num2str(minimadist) '_overlapthresh' num2str(overlapthresh) '.func.gii']);
