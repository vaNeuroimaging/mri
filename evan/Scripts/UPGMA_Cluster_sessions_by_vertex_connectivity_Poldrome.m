...
for hemnum = 1:length(hems)
    ...
    load(['/data/cn4/evan/RestingState/Consensus/Poldrome/Network_' hem '_avgconnectivity_xdist20.mat'])
    ...
    outputfolder = ['/data/cn4/evan/RestingState/Ind_variability/Poldrome/Session_clustering_vertexwise/'];
    ...
    [subjects subdata ign] = textread(datalist,'%s%s%s');
...
%             if divisionnum==1
%                 evalc(['!wb_command -cifti-convert -to-gifti-ext ' subdata{subnum} ' Session' num2str(subnum) '.func.gii']);
%             end
%             subtimecourse = gifti(['Session' num2str(subnum) '.func.gii']); subtimecourse = subtimecourse.cdata;
        ...
        for vertexnum = 1:length(divisionindices)
            ...
            Y = pdist(squeeze(subjectcorrelpatterns(:,vertexnum,:)),'correlation');
