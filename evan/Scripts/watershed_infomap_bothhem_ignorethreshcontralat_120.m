...
outputfolder = [waterdir '/120_LR_ignorethreshcontralat_infomap/'];
...
cl_xdistancemat = ones(size(all_water_corrmat));
...
for hem = 1:length(HEMS)
    ...
    cl_xdistancemat(heminds,heminds) = hemxdistancemat;
    
    hemxdistancemat = surf_distances{otherhem}(parcelcenterinds{hem},parcelcenterinds{otherhem});
    hemxdistancemat = hemxdistancemat > distanceexclusion;
    cl_xdistancemat(heminds,otherheminds) = hemxdistancemat;
    
end
clear surf_distances

all_water_corrmat = all_water_corrmat .* cl_xdistancemat;
kdens_totest = [kdenthresholds(1) : kdeninterval : kdenthresholds(2)];
for kdennum = 1:length(kdens_totest);
    thiskden = kdens_totest(kdennum);
    [matrix rs(kdennum) kden] = matrix_thresholder(all_water_corrmat,thiskden,'kden');
end
...
dlmwrite(prmfilename,num2str(1),'-append','delimiter','');
dlmwrite(prmfilename,num2str(rs(1)),'-append','delimiter','');
dlmwrite(prmfilename,num2str(rint_approx),'-append','delimiter','');
dlmwrite(prmfilename,num2str(rs(end)),'-append','delimiter','');
...
%% Run infomap
cd(outputfolder)
graphcluster_Evan_surfacexd_customthresh([outputfolder '/prmfile.txt'],'thr',distanceexclusion,xdistancemat,rs',1,0,'infomap');
