allparcelfilenames = {'/data/cn4/evan/ROIs/Shen/Shen_300_L.func.gii','/data/cn4/evan/ROIs/Shen/Shen_300_R.func.gii';...
...
for parcellation = 3:size(allparcelfilenames,1)
    

%waterdir = '/data/cn4/evan/RestingState/FC_Mapping_120/subsurf/nosmooth/';%'/data/cn4/evan/RestingState/FC_Mapping_120/';
%watershednames = {'120_subsurf_L_nosmooth_watershedmerge_0.4_tweaked.func.gii','120_subsurf_R_nosmooth_watershedmerge_0.4_tweaked.func.gii'};%,'Poldrome_R_crossthresh_watershedmerge.func.gii'};
corrmatname = 'corrmat.mat';
roifilename = 'parcel_center.roi';

slashlocs = strfind(allparcelfilenames{parcellation,1},'/');
shortname = allparcelfilenames{parcellation,1}(slashlocs(end)+1 : end);
Lloc = strfind(shortname,'_L');
shortname = shortname([1:(Lloc-1) (Lloc+2):(end-9)]);

outputfolder = ['/data/cn4/evan/RestingState/FC_Mapping_120/' shortname '_bothhem_infomap/'];
mkdir(outputfolder);

kdenthresholds = [.01 .03];
kdeninterval = .001;
distanceexclusion = 20;
networksizeminimum = 5;

outputstem = shortname;
...
% 
        ...
        watershed = gifti(allparcelfilenames{parcellation,hem});
...
for hem = 1:length(HEMS)
    ...
    water = gifti(allparcelfilenames{parcellation,hem});
    water = water.cdata;
...
%% Run infomap
cd(outputfolder)
graphcluster_Evan_surfacexd([outputfolder '/prmfile.txt'],'thr',distanceexclusion,xdistancemat,0,0,'infomap');
...
save(gifti(single(water_com_assigns)),[outputstem HEMS{hem} '_minsize' num2str(networksizeminimum) '.func.gii']);
 ...
 clear water_corrmat folderdatecreated waternum
...
%  tallymat_infomap(assns(:,6:26),[outputfolder '/' roifilename]);
% simple_assigns = modify_clrfile_nopic('simplify','tallymat_assn.txt',networksizeminimum);
%  
% 
%  for hem = 1:length(HEMS)
% 
...
%       
%  if length(HEMS) > 1
%      cifti_write_wHDR(ciftidata_towrite,ciftitemplatefile,['tallyconsensus_' outputstem 'LR_minsize' num2str(networksizeminimum)])
%  end
