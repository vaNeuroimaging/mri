...
surfdatadir = '/data/cn4/evan/RestingState/FC_Mapping_120/nosmooth/';
outputdir = '/data/cn4/evan/RestingState/FC_Mapping_120/nosmooth/';
%tmasktype = '/data/cn4/evan/RestingState/FC_Mapping_120/Old/C1toC3_DATALIST_TMASK.txt';
smooth = 'no';
hems = 'BOTH';
edges = 'yes';
...
[funcdata tmaskfiles subjects] = textread(cohortfile,'%s %s %s');
subnum = length(subjects);
for s=1:subnum
    slashloc = strfind(funcdata{s},'/');
    extensionloc = strfind(funcdata{s},'.4dfp.img');
    funcpaths{s,1} = funcdata{s}(1:slashloc(end));
    funcvols{s,1} = funcdata{s}(slashloc(end)+1:extensionloc);
end
...
for hem = h
    ...
    switch smooth
        case 'no'
            filestem = ['avgcorrofcorr_nosmooth_allgrad_' HEMS{hem}];
        case 'yes'
