...
outputfolder = '/data/cn4/evan/RestingState/Ind_variability/Subjects/Assignments/Merged_tomatch/';
...
centroid_distance_map_outputname = ['Assignment_median_centroid_distance'];
...
countmatches = zeros(length(templatedata.cdata),1);
...
for hemnum = 1:length(hems)
    ...
    load(['/data/cn4/evan/fsaverage_LR32k/Surface_distances_' hem '.mat']);
     inds = [1:length(geo_distances)] + (length(geo_distances)*strcmp(hem,'R'));
...
%     g2sassignments_cifti = g2sassignments(logical(mask),:);
    ...
    randomizedgroup = gifti(['/data/cn4/evan/RestingState/FC_Mapping_120/120_' hem '_wateredgethresh_watershedmerge_0.45_randomized.func.gii']);
    randomizedgroup = randomizedgroup.cdata;
    randomizedgroup_cifti = randomizedgroup(logical(mask));
    ...
    for parcelnum = 1:length(parcelIDs)
        ...
        for s = 1:length(subjects)
            parcelmatchID = mean(randomizedgroup_cifti(logical(groupparcels_cifti==parcelIDs(parcelnum))));
            
            s2gassignments = gifti(['/data/cn4/evan/RestingState/Ind_variability/Subjects/' subjects{s} '/' subjects{s} '_' hem '_watershedmatchtogroup.func.gii']);
            s2gassignments = s2gassignments.cdata;
%               s2gassignments = gifti([outputfolder 'Subject_parcels_matchtogroup4_' hem '.func.gii']);
%               s2gassignments = s2gassignments.cdata(:,s);
% 
            s2gassignments_cifti = s2gassignments(logical(mask));
            
            
            ind = find(s2gassignments==parcelmatchID);
            ...
            if isempty(ind)
                ...
                countmatches(thishem_groupparcelindices{parcelnum,1}) = countmatches(thishem_groupparcelindices{parcelnum,1})+1;
            ...
            thishem_subparcelindices{parcelnum,s} = find(s2gassignments_cifti==parcelmatchID) + (strcmp('R',hem) * ncortexLverts);
        end
...
cifti_write_wHDR(countmatches,templatefile,[outputfolder '/Nummatches'])
...
for subi = 1:length(subjects)
    for subj = 1:length(subjects)
        if subi > subj
            ...
            xdistmati = geo_distances_both(notnan_centroidsi,notnan_centroidsi) > distanceexclusion;
            xdistmatj = geo_distances_both(notnan_centroidsj,notnan_centroidsj) > distanceexclusion;
            notnans = logical(parcelsmatched .* xdistmati(:) .* xdistmatj(:));
            ...
            xdistmati = geo_distances_both(groupparcel_centroid,groupparcel_centroid) > distanceexclusion;
            %xdistmatj = xdistmati;
            notnans = logical(parcelsmatched .* xdistmati(:));
...
figure
imagesc(group_subtosubcorrelmat,[.4,.7])
colormap hot
colorbar

mean(subcorrelvals)
figure
imagesc(sub_subtosubcorrelmat,[.4,.7])
colormap hot
colorbar

figure
imagesc(sub_subtosubcorrelmat - group_subtosubcorrelmat,[-.1,.1])
