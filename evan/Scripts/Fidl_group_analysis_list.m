...
%GLM List
list = '/data/cn4/katie/DEV_PHONOLOGY_August2010/REPREPROCESS/GLM_lists/glmv1R_32adults.glm_list';
...
%
%'/data/cn4/evan/Task_parcellation/Activations/MRTNV_26.glm_list';
...
% ANOVAsetup(:,:,1) = {[1] [6]; [11] [16]; [21] [26]};
% ANOVAsetup(:,:,2) = {[2] [7]; [12] [17]; [22] [27]};
% ANOVAsetup(:,:,3) = {[3] [8]; [13] [18]; [23] [28]};
% ANOVAsetup(:,:,4) = {[4] [9]; [14] [19]; [24] [29]};
% ANOVAsetup(:,:,5) = {[5] [10]; [15] [20]; [25] [30]};
%[4 5 6 7],[12 13 14 15 16 17],[23 24]};
...
%within each factor. 
levelnames = {{'RealWords' 'Pseudowords'} {'Regularize' 'Read'}};


%The directory that results will be written to.
outputdir = '/data/cn4/evan/Occipitotemporal/Katie/';
...
for subject = 1:length(glmfiles)
    ...
    %---------------------------------------------------------------------
     ...
     subjzstatrunstr = [subjzstatrunstr '-xform_file ' T4files{subject} ' -gauss_smoth 2 -compress /home/usr/fidl/lib/glm_atlas_mask_' voxelspace '.4dfp.img -atlas ' voxelspace ' -scratchdir ' outputdir '/' Scratchdir];
     singlesubjrunstr = [singlesubjrunstr '-xform_files ' T4files{subject} ' -gauss_smoth 2 -mask /home/usr/fidl/lib/glm_atlas_mask_' voxelspace '.4dfp.img -atlas ' voxelspace ' -directory ' outputdir '/SingleSubjects/  -frames ' num2str(timepoints) ' -glmpersub 1 -tc_names '];
