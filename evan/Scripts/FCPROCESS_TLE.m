function [allends switches] = FCPROCESS_TLE(datafile,tdir,tmasktype,cleanup,roilist,varargin)
%
% Name:FCPROCESS.m
% $Revision: 1.4 $
...
for i=1:size(rois,1)
    roimasks(:,i)=read_4dfpimg_HCP(rois{i,1});
end
end
    ...
    switch switches.regressiontype
        case 0 % classic WashU seeds in WM and V
            ...
            for i=1:numsubs
                ...
                QC(i).GLMmaskfile=['/data/cn4/evan/ROIs/glm_atlas_mask_MNI_' voxdim '.4dfp.img'];
                %QC(i).WMmaskfile=['/home/usr/fidl/lib/small_WM.4dfp.img'];
                QC(i).WMmaskfile=['/data/cn4/evan/ROIs/small_WM_MNI_333.4dfp.img'];
                %QC(i).CSFmaskfile=['/home/usr/fidl/lib/711-2B_lat_vent_333.4dfp.img'];
                QC(i).CSFmaskfile=['/data/cn4/evan/ROIs/711-2B_lat_vent_MNI_333.4dfp.img'];
            end
        ...
        switch switches.regressiontype
            case 0
                ...
                write_4dfpimg(QC(i).seedhold,[ QC(i).subseeddir '/usedseeds.4dfp.img'],'bigendian');
                 write_4dfpifh_333_MNI([ QC(i).subseeddir '/usedseeds.4dfp.img'],size(QC(i).seedhold,2),'bigendian');
...
for f=1:numsubs
    ...
    ending='333';
    numvox = numel(QC(i).GLMMASK);
    [bolds]=conc2bolds([LASTCONC{stage} '.conc']);
    [tempimg]=bolds2img(bolds,tr(i).tot,tr(i).start,numvox);
    ...
    if switches.doregression
        ...
        write_4dfpimg(zb,'beta.4dfp.img','bigendian');
         write_4dfpifh_333_MNI('beta.4dfp.img',size(zb,2),'bigendian');
...
%     end
    ...
    if switches.doblur
        ...
        for j=1:numel(QC(i).restruns)
            ...
            write_4dfpimg(temprun,[ LASTIMG{i,j,stage-1} '.4dfp.img'],'bigendian');
             write_4dfpifh_333_MNI([ LASTIMG{i,j,stage-1} '.4dfp.img'],size(temprun,2),'bigendian');
        end
        ...
        pre_file = read_4dfpimg_HCP(pre_file_name);
    ...
    write_4dfpimg(tempimg,[ QC(i).vcnum '_' allends '.4dfp.img'],'bigendian');
     write_4dfpifh_333_MNI([ QC(i).vcnum '_' allends '.4dfp.img'],size(tempimg,2),'bigendian');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [fcimg]=bolds2img(bolds,trtot,trborders,vox)
%vox=147456;
...
if nargin==3
    ...
    write_4dfpimg(tempbold,outname,'bigendian');
     write_4dfpifh_333_MNI(outname,size(tempbold,2),'bigendian');
end
