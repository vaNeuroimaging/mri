function watershed_algorithm_submatchtogroup_merge(edgemetricname,outputdir,hem,subname)
...
origsubparcels = subparcels;
    ...
    %%
    zeroinds = find((subassignmap==0) .* (mask==1));
    
    for ind = zeroinds(:)'
        indneighs = neighbors(ind,2:7); indneighs(isnan(indneighs)) = [];
        nonzeroneighvals = unique(subassignmap(indneighs)); nonzeroneighvals(nonzeroneighvals==0) = [];
        if length(nonzeroneighvals) == 1
            subassignmap(ind) = nonzeroneighvals;
        end
    end
...
for waternum = 1:length(watersheds)
    ...
    IDs(waternum) = mean(subassignmap(label==watersheds(waternum)));
...
for waternum = 1:length(sortedIDs)
    if any(sortedIDs(1:waternum-1) == sortedIDs(waternum));
        subassignmap(label==sortedclusters(waternum)) = 0;
    end
end
...
save(gifti(subassignmap),[outputdir '/' filestem '_' hem '_watershedmatchtogroup.func.gii']);
