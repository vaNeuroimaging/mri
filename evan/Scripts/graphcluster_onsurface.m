function graphcluster_onsurface(prmfile,analysistype,xdistance,makebinary,savematfile,clustertype,varargin)
%
% Name:graphcluster.m
% $Revision: 1.2 $
...
nodecoordsfile = '/data/cn4/evan/fsaverage_LR32k/VertexCoords.L.txt';
...
if ~isempty(xdistance)
    if isnumeric(xdistance) && (xdistance>=0)
        ...
        nodecoords = load(nodecoordsfile);
        distances = euclidean_distance(nodecoords);
        
        [matrix] = matrix .* (distances >= xdistance);
...
for i=1:numanalyses
    for j=1:finaldim
        ...
        [rmat foundr(i,j) foundkden(i,j)] = matrix_thresholder(rmat,thresholdarray(i,1),thresholdtype);
        ...
        switch clustertype
            ...
            case 'infomap'
                ...
                %cd /data/cn/data1/scripts/graphtools/bin
