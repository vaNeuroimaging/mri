function outputmetric = metric_cluster_surfacearea(metricname,minthresh,maxthresh,minsize,hem,surfaceareafile,varargin)
%outputmetric = metric_cluster_surfacearea(metricname,minthresh,maxthresh,minsize,hem,surfaceareafile,[outputstem])
...
if exist('surfaceareafiles') && (~isempty(surfaceareafile))
    surfacearea = gifti(surfaceareafile);
else
...
for clusternum = 1:length(uniqueclustervals)
    ...
    if sum(surfacearea(clusteredmetric==uniqueclustervals(clusternum))) > minsize
        %length(find(clusteredmetric==uniqueclustervals(clusternum))) > minsize
