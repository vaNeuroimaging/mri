...
outputdir = '/data/cn4/evan/Task_parcellation/HCPSubjects/Parcellation_Group56_46beta_subregister/';
datadir = '/data/cn4/evan/Task_parcellation/HCPSubjects/';
smooth = 'no';
hems = 'RIGHT';
edges = 'yes';
...
subjectlist = '/data/cn4/evan/Task_parcellation/HCPSubjects/SubjectList2.txt';

subjects = textread(subjectlist,'%s');


betalists = {'/data/cn4/evan/Task_parcellation/HCPSubjects/MainTaskContrasts.txt'};


for beta = 1:length(betalists)
    clear betastouse
    betalist = betalists{beta};
    
    %get the shortened name of this beta list
    betalistunderscorloc = strfind(betalist,'_no');
    betalistsuffix = betalist(betalistunderscorloc+1:end-4);
    
    %read the list of betas to use
    [ign betanums ign ign ign ign] = textread(betalist,'%s%s%s%s%s%s');
    
    %save the beta numbers to be used
    ...
    %Read the HCP brain mask
    mask4dfp='/data/cn4/evan/Task_parcellation/HCPSubjects/MNI152_T1_2mm_brain_mask.4dfp.img';
    volmask = read_4dfpimg_HCP(mask4dfp);
    ...
    for hem = h
        ...
        medialmask = ['/data/cn4/laumannt/32k_ConteAtlas_v2/medial_wall.' HEMS{h} '.32k_fs_LR.func.gii'];
        ...
        for divisionnum = 1:divisions
            ...
            for s = 1:length(subjects)
                ...
                surfdir = ['/data/cn4/evan/HCP_Structural/' subjects{s} '/MNINonLinear/'];
                
                midsurf = [surfdir '/Native/' subjects{s} '.' HEMS{h} '.midthickness.native.surf.gii'];
                whitesurf = [surfdir '/Native/' subjects{s} '.' HEMS{h} '.white.native.surf.gii'];
                pialsurf = [surfdir '/Native/' subjects{s} '.' HEMS{h} '.pial.native.surf.gii'];
                
                %Get this subject's surface and volume task data 
                %surfdata = [datadir '/' subjects{s} '_mergeFile_L_32k_fsLR.func.gii'];
                voldata = [datadir '/' subjects{s} '_mergeFile.nii.gz'];
...
%                     evalc(['!awk ''NF > 25'' ' surfdata ' > ' surfdata(1:end-9) '_noHEAD.func.gii']);
                    gunzip(voldata);
                    evalc(['!nifti_4dfp -4 ' voldata(1:end-7) '.nii ' voldata(1:end-7)]);
                    
                    evalc(['!' workbenchdir '/wb_command -volume-to-surface-mapping '  voldata(1:end-7) '.nii ' midsurf ' ' voldata(1:end-7) '_' HEMS{h} '_native.func.gii -ribbon-constrained ' whitesurf ' ' pialsurf ' -voxel-subdiv 5']);
                    evalc(['!' workbenchdir '/wb_command -metric-smoothing ' midsurf ' ' voldata(1:end-7) '_' HEMS{h} '_native.func.gii ' num2str(smoothnum) ' ' voldata(1:end-7) '_' HEMS{h} '_native_smooth.func.gii']);
                    
                    cd([surfdir '/fsaverage_LR32k/'])
                    evalc(['!caret_command64 -deformation-map-apply native232k_fs_LR.' HEMS{h} '.deform_map METRIC_AVERAGE_TILE '  voldata(1:end-7) '_' HEMS{h} '_native_smooth.func.gii '  voldata(1:end-7) '_' HEMS{h} '_32k_fsLR.func.gii']);
                    cd(datadir)
                    
                    evalc(['!caret_command64 -file-convert -format-convert XML ' voldata(1:end-7) '_' HEMS{h} '_32k_fsLR.func.gii']);
                    
                end
                
                %Read in the surface data and get the betas to be parcellated
                %surf_BOLD = load([surfdata(1:end-9) '_noHEAD.func.gii']);
                surf_BOLD = gifti([voldata(1:end-7) '_' HEMS{h} '_32k_fsLR.func.gii']);
                surf_BOLD = surf_BOLD.cdata(:,betastouse);
                
                %Read in the volume data and get the betas to be parcellated 
                voldata4dfp = [voldata(1:end-7) '.4dfp.img'];
                BOLD = read_4dfpimg_HCP(voldata4dfp);
                vol_BOLD = BOLD(logical(volmask),betastouse)';
            ...
            save([outputdir '/CrosscorrDivision_' betalistsuffix '_' num2str(divisionnum) '.mat'],'avgcrosscorr','-v7.3')
        ...
        delete *_native.func.gii
        delete *_native_smooth.func.gii
        ...
        for i = 1:divisions
            ...
            load([outputdir '/CrosscorrDivision_' betalistsuffix '_' num2str(i) '.mat']);
            ...
            for j = 1:divisions
                ...
                if i >= j
                    ...
                    load([outputdir '/CrosscorrDivision_' betalistsuffix '_' num2str(j) '.mat']);
        ...
        system(['rm ' outputdir '/CrosscorrDivision_' betalistsuffix '_*.mat']);
        
        cd(outputdir)
        
        
        load /data/cn4/evan/Task_parcellation/SteveSubjects/Parcellation_HCPandSteve_bysubs/avgcorrofcorrStevedata.mat;
        
        avgcorrofcorr = (avgcorrofcorr + avgcorrofcorrStevedata) ./ 2;
        ...
        filestem = ['avgcorrofcorr_smooth' num2str(smoothnum) '_allgrad_' HEMS{hem} '_' betalistsuffix];
