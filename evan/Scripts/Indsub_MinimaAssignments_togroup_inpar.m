...
outputstatsfile = [outputfolder '/Statistics_manydist_manydistthresh_10.txt'];
...
outputmetric = zeros(32492,length(minimadistances), length(distthreshvals));

%matlabpool open 4

parfor minnum = 1:length(minimadistances)
    minimadistances(minnum) = minimadistances(minnum);
    
%     [ign subnum] = system(['cat ' cohortfile ' | wc -l']);
% subnum = str2num(subnum);

%    groupminimafile = [groupedgefile(1:end-9) '_minima_dist' num2str(minimadistances(minnum)) '.func.gii'];
    
%disp('Calculating group minima')
% if makenewgroupstuff
%     metric_minima([groupedgemapfolder '/' groupedgefile],minimadistances(minnum),[],hem,outputfolder,groupminimafile);
% end

groupminimadata = gifti([outputfolder '/' groupedgefile(1:end-9) '_minima_dist' num2str(minimadistances(minnum)) '.func.gii']);
groupminimadata = groupminimadata.cdata;
groupminimadata(medialmaskind) = 0;
groupminimavertices = find(groupminimadata);

disp('Getting distances from group minima')
% if makenewgroupstuff
%     
...
%         system('caret_command64 -file-convert -format-convert XML temp.func.gii');
%         thisdistances = gifti('temp.func.gii');
%         distances(minimanum,:) = thisdistances.cdata;
%     end
%     disp(' ')
%     save([outputfolder '/distances_' num2str(minimadistances(minnum))],'distances')
%     system('rm temp.func.gii');
% else
   distances = load([outputfolder '/distances_' num2str(minimadistances(minnum))]);
...
disp('Getting average connectivity patterns of group minima');
...
%     save([outputfolder '/avgcorrelmat_' num2str(minimadistances(minnum))],'avgcorrelmat');
% else
     avgcorrelmat = load([outputfolder '/avgcorrelmat_' num2str(minimadistances(minnum))]);
     avgcorrelmat = avgcorrelmat.avgcorrelmat;
% end
...
for s = 1:subnum
    ...
    subjectminimametric = [subjectedgemetric(1:end-9) '_minima_dist' num2str(minimadistances(minnum)) '.func.gii'];
    
    try subjectminimadata = gifti([outputfolder '/' subjectminimametric]);
    catch
        metric_minima([edgemapfolder '/' subjectedgemetric],minimadistances(minnum),[],hem,outputfolder,subjectminimametric);
...
%save(gifti(single(squeeze(outputmetric(:,:,distthreshnum)))),[outputfolder '/SubjectsMinimaAssignments_dist' num2str(minimadistances(minnum)) '_param' num2str(costparameter) '_distthresh' num2str(distthresh) '_lapjv.func.gii'],'ExternalFileBinary');
%end
%clear outputmetric
