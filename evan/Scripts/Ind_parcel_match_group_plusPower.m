...
power_correlpatterns_file = '/data/cn4/evan/RestingState/FC_Mapping_120/Power_surf_ROIs_mean_correlpatterns.dtseries.nii';
...
for hem = 1:length(HEMS)
    ...
    thispowerparcels = gifti(['/data/cn4/evan/ROIs/264_surfvert_ROIs_' HEMS{hem} '.func.gii']);
    powerparcels{hem} = thispowerparcels.cdata;
    ciftispace_powerparcels{hem} = powerparcels{hem}(logical(thismedialwall));
    
    powerparcelIDs{hem} = unique(powerparcels{hem}); powerparcelIDs{hem}(powerparcelIDs{hem}==0) = [];
    
    for parcelnum=1:length(powerparcelIDs{hem})
        powerparcelcount = powerparcelcount+1;
        powerparcelindices{powerparcelcount} = find(ciftispace_powerparcels{hem}==powerparcelIDs{hem}(parcelnum)) + (strcmp(HEMS{hem},'R') * ncortexLverts);
        
        gifti_powerparcelindices{hem}{parcelnum} = find(powerparcels{hem}==powerparcelIDs{hem}(parcelnum));
        
        ind = gifti_powerparcelindices{hem}{parcelnum};
        meanX = mean(sphere.vertices(ind,1));
        meanY = mean(sphere.vertices(ind,2));
        meanZ = mean(sphere.vertices(ind,3));
        coord = [meanX meanY meanZ];
        ...
        if hem==1
            power_validcorrelverts{powerparcelcount} = [(geo_distances_byhem{hem}(logical(thismedialwall),centroidvert) > xdistance); ones(ncortexRverts,1); ones(nsubcortverts,1)];
        else
            power_validcorrelverts{powerparcelcount} = [ones(ncortexLverts,1); (geo_distances_byhem{hem}(logical(thismedialwall),centroidvert) > xdistance); ones(nsubcortverts,1)];
        end
    end
    
end
numparcels = parcelcount;
numpowerparcels = powerparcelcount;
...
if make_avg_correlpatterns
    ...
    mean_powerparcel_correlpatterns = cifti_read(power_correlpatterns_file);
end
...
for s = 1:length(LFRSsubjects)
    ...
    for parcelnum = 1:numpowerparcels
        parcel_timecourse = mean(surf_timecourse(powerparcelindices{parcelnum},:),1);
        
        parcel_correlpattern = paircorr_mod(surf_timecourse',parcel_timecourse');
        parcel_correlpattern(isnan(parcel_correlpattern)) = 0;
        
        powerparcel_subcorrelpatterns(:,parcelnum) = parcel_correlpattern;
        
        clear parcel_correlpattern
        
        powertopower_correls(s,parcelnum) = paircorr_mod(powerparcel_subcorrelpatterns(logical(power_validcorrelverts{parcelnum}),parcelnum),mean_powerparcel_correlpatterns(logical(power_validcorrelverts{parcelnum}),parcelnum));
    end
    ...
    for hem = 1:length(HEMS)
        ...
        g2sassignments = gifti(['/data/cn4/evan/RestingState/Ind_variability/Subjects/Assignments/Matching_expand/Group_parcels_matchtosubs2_' HEMS{hem} '.func.gii']);
        g2sassignments = g2sassignments.cdata;
        g2sassignments_cifti = g2sassignments(logical(thismedialwall),:);
        s2gassignments = gifti(['/data/cn4/evan/RestingState/Ind_variability/Subjects/Assignments/Matching_expand/Subject_parcels_matchtogroup2_' HEMS{hem} '.func.gii']);
...
plot(centers,n/numel(grouptoadjacent_correls),'k-');

hold on
[n x] = hist(grouptogroup_correls(:),centers);
plot(centers,n/numel(grouptogroup_correls),'b-');

[n x] = hist(powertopower_correls(:),centers);
plot(centers,n/numel(powertopower_correls),'g-');
