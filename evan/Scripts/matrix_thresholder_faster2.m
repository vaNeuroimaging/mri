function [i kden r] = matrix_thresholder_faster2(matrix,threshold,thresholdtype,roifile)
%
...
%         end
        ...
        if d(1)<100000 % if it is a "smaller" matrix that won't destroy macthunder's RAM
            ...
            [~, i]=sort(matrix_triu(:),'descend');
            i = i(1:(round(threshold(end)*numpossibleedges)));
            mask_matrix_triu = false(size(matrix_triu));
            mask_matrix_triu(i) = 1;
            mask(mask) = mask_matrix_triu;
            i = find(mask);
            [~,order] = sort(matrix(i),'descend');
            i = i(order);
            for t = 1:size(threshold,2)
                edgesleft=round(threshold(t)*numpossibleedges);
             %   r(t) = v(edgesleft);
                kden(t)=edgesleft/numpossibleedges;
            end
