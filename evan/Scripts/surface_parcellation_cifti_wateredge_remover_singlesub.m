function  surface_parcellation_cifti_wateredge_remover_singlesub(cohortfile,topoutputdir,tmasktype,hems,edges)
...
roidir = '/data/hcp-bluearc/home/laumannt/120_parcellation';
HEMS = {'L';'R'};
hemname = {'LEFT';'RIGHT'};
...
slashloc = strfind(surfdatafile{s},'/');
funcpaths{s,1} = surfdatafile{s}(1:slashloc(end));
cifti_files{s,1} = surfdatafile{s}(slashloc(end)+1:end-13);
...
for s = [2:length(subjects) 1]
    
    outputdir = [topoutputdir '/' subjects{s} '/'];
...
%        surfdir = strcat(surfdirs{s});     
        ...
        disp('done')
        system(['mv ' outputdir '/' cifti_file '_corr_temp.dconn.nii ' outputdir '/' cifti_file '_' HEMS{hem} '_corr.dconn.nii'])
...
%     end
    ...
    system([workbenchdir '/wb_command -cifti-correlation ' outputdir '/' cifti_file '_' HEMS{hem} '_corr.dconn.nii ' outputdir '/avg_corrofcorr_' HEMS{hem} '.dconn.nii'])
    disp('done')
    ...
    medial_wall = gifti(['/data/hcp-bluearc/home/laumannt/120_parcellation/' HEMS{hem} '.atlasroi_group_noproj.func.gii']);   
