function cleaned = Cifti_clean(data,minsize,surfaceareafile)
%cleaned = Cifti_clean(data,minsize,surfaceareafile)
...
if ~exist('surfaceareafile')
    surfacearea = cifti_read('/data/cn4/evan/fsaverage_LR32k/Conte69.LR.midthickness.32k_fs_LR_surfaceareas_normalwall.dtseries.nii');
else
    surfacearea = cifti_read(surfaceareafile);
end
...
for col = 1:size(data,2)
    ...
    for ID = IDs(:)'
        ...
        for clusternum = 1:length(uniqueclustervals)
            ...
            if sum(surfacearea(clustereddata==uniqueclustervals(clusternum))) < minsize
