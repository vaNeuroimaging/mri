function watershed_algorithm_matchtogroup_merge(edgemetricname,datafile,tmaskfile,outputdir,hem,outfilestem)
%watershed_algorithm_matchtogroup_merge(edgemetricname,datafile,tmaskfile,outputdir,hem,outfilestem)
...
origsubparcels = subparcels;
    ...
    if strcmp(datafile(end-12:end),'.dtseries.nii')
        data = cifti_read(datafile);
        tmask = load(tmaskfile);
        data = data(:,logical(tmask));
        timecourse = 1;
    elseif strcmp(datafile(end-9:end),'.dconn.nii')
        data = cifti_read(datafile);
    ...
    for subparcelnum = 1:length(subparcelIDs)
        ...
        subparcelindices{subparcelnum} = find(subparcels==subparcelIDs(subparcelnum)) + (strcmp(hem,'R') * ncortexLverts * timecourse);
        
        if timecourse == 1;
            parceltimecourse = mean(data(subparcelindices{subparcelnum},:),1);
            subparcelcorrelpattern = paircorr_mod(parceltimecourse',data');
            subparcelcorrelpattern(isnan(subparcelcorrelpattern)) = 0;
        else
            subparcelcorrelpattern = mean(data(subparcelindices{subparcelnum},:),1);
        end
...
save(gifti(edgethresh_subassignmap),[outputdir '/' outfilestem '_' hem '_watershedmatchtogroup2.func.gii']);
