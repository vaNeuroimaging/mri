function [hout,T,perm] = dendrogram_evan(Z,cmap_init,sizemin,varargin)
...
groupIDs = unique(theGroups); groupIDs(groupIDs==0) = [];
for i = 1:length(groupIDs)
    sizes(i) = nnz(theGroups==groupIDs(i));
end
[sorted sortind] = sort(sizes,'descend');


cmap = zeros(length(groupIDs),3);
count = 1;
for i = 1:length(groupIDs)
    if sorted(i) >= (sizemin-1)
        cmap(sortind(i),:) = cmap_init(count,:);
        count = count+1;
    else
        cmap(sortind(i),:) = [0 0 0];
    end
end
