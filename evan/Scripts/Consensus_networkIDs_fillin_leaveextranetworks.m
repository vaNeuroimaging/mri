function out = Consensus_networkIDs_fillin_leaveextranetworks(regularizedfilename,distances)
...
for col = 1:size(subnetworks,2)
    string{col} = ['Assigning network IDs for column ' num2str(col) ' of ' num2str(size(subnetworks,2))];
    if col==1; 
        fprintf('%s',string{col}); 
    elseif col==size(subnetworks,2); 
        fprintf([repmat('\b',1,length(string{col-1})) '%s'],string{col}); 
        disp(' '); 
    else fprintf([repmat('\b',1,length(string{col-1})) '%s'],string{col}); 
    end
    ...
    pctoverlapverts = zeros(length(groupIDs),length(subIDs));
    for groupIDnum = 1:length(groupIDs)
        ...
        for subIDnum = 1:length(subIDs)
            ...
            pctoverlapverts(groupIDnum,subIDnum) = nnz((surfgroupnetworks==groupID) .* (surfsubnetworks==subID)) / nnz((surfgroupnetworks==groupID));
            
        end
        
    end
    
    %Prevent low-overlap matches 
    mean_mindists(pctoverlapverts<overlapcutoff) = Inf;
    
    %Assign via Hungarian algorithm
    [colassigns{col},ign] = munkres(mean_mindists);
    for i = 1:length(colassigns{col})
        if (colassigns{col}(i) > 0) && (pctoverlapverts(i,colassigns{col}(i)) < overlapcutoff)
            colassigns{col}(i) = 0;
        end
    end
...
for col = 1:size(subnetworks,2)
    ...
    string{col} = ['Assigning unassigned networks and cleaning maps for column ' num2str(col) ' of ' num2str(size(subnetworks,2))];
    if col==1; 
        fprintf('%s',string{col}); 
    elseif col==size(subnetworks,2); 
        fprintf([repmat('\b',1,length(string{col-1})) '%s'],string{col}); 
...
%     subIDsassigned = zeros(length(colassigns{col}),1);
%     for i = 1:length(colassigns{col})
%         if colassigns{col}(i) > 0
%             subIDsassigned(i) = allsubIDs{col}(colassigns{col}(i));
%         end
%     end
%     
%     stillunassignedIDs = [];
...
%             
%             if (filledsubnetworks(vert,thiscol) > 0) && any(filledsubnetworks(vert,thiscol)==subIDsassigned)
...
%             
%             
% %                 subIDsassigned = zeros(length(colassigns{thiscol}),1);
% %                 for i = 1:length(colassigns{thiscol})
% %                     if colassigns{thiscol}(i) > 0
% %                         subIDsassigned(i) = allsubIDs{thiscol}(colassigns{thiscol}(i));
% %                     end
% %                 end
% %                 
% %                 assign_nozeros = colassigns{thiscol}; assign_nozeros(assign_nozeros==0) = [];
% %                 
% %                 if any(filledsubnetworks(vert,thiscol)==allsubIDs{thiscol}(assign_nozeros))
% %                     matched(vert) = groupIDs(subIDsassigned==filledsubnetworks(vert,thiscol));
% %                     break
% %                 end
% %                 if thiscol==size(filledsubnetworks,2)
...
%     end
    ...
    for extraIDnum = 1:length(unassignedIDs)%length(stillunassignedIDs)
        recoloredverts(filledsubnetworks(:,col)==unassignedIDs(extraIDnum)) = extraassignments(extraIDnum);
    end
    matched(vertstofillin) = recoloredverts(vertstofillin);
