...
mapname = 'Cluster_probability_maps_sorted_10mm';%'Cluster_probability_maps_sorted_10mm_36sub';%'Cluster_probability_maps_sorted_10mm_40sub';
sizesname = 'Cluster_probability_maps_sorted_10mm_mediansizeoutlines';%'Cluster_probability_maps_sorted_10mm_mediansizeoutlines_36sub';%;'Cluster_probability_maps_sorted_10mm_mediansizeoutlines_40sub';%
interestingfeature = 'notintemplate_plusextended';

%networksfound = {'Default','Visual','FrontoPar','DorsalAttn','VentAttn','Salience','CingOperc','MortorHand','MotorMouth','Auditory','MedPar','ParOccip'};
...
%     for j = 1:size(outputclusters,2)
%         dice(j) = nnz(logical(outputclusters(:,j)) & (thismap>.05)) ./ nnz(logical(outputclusters(:,j)) | (thismap>.05));
%     end
%     [~,maxdiceind] = max(dice);
...
%'Cluster_probability_maps_sorted_10mm_40sub_intemplate.dscalar.nii');
%
%
...
for mapnum = 1:size(maps.data,2)
    ...
    %[system ' ' nsubs ' ' mapsize ' ' hem ' ' maxcoords ' ' maxlabel];
    
    dlmwrite(outputfilename,texttowrite,'-append','delimiter','')
end


%% Subject patches from probability map

probmapfile = 'Cluster_probability_maps_sorted_10mm_40sub_notintemplate.dscalar.nii';
columns = [19];
...
for communitynum = 1:length(communities)
    ...
    if rs(communitynum)>.999
        ...
        tokens = tokenize(probmaps.mapname{column},':');
...
ft_write_cifti_mod(['Submaps_column' num2str(column) '_' probmapfile],cifti_template);
