function generate_rotated_parcellation_andPCA_nobaddatastrict_Poldrome(watershedname,iterations,cov_corr,rotations,iscifti,hem,outfileprefix)
...
for parcelID = parcelIDs'
    if (nnz(realwatershed(allbadvertices)==parcelID) > minsize)  || (nnz(realwatershed==parcelID) < 15)
...
geo_distances = smartload(['/data/hcp-zfs/home/laumannt/Poldrome/shared_for_washu/freesurfer_washu/FREESURFER_fs_LR/sub013/7112b_fs_LR/fsaverage_LR32k/geodesic_distance_' hem '.mat']);
%geo_distances_frombad = geo_distances(allbadvertices,:);



surfdir = '/data/hcp-zfs/home/laumannt/Poldrome/shared_for_washu/freesurfer_washu/FREESURFER_fs_LR/sub013/7112b_fs_LR/fsaverage_LR32k/';%'/data/cn4/laumannt/standard_mesh_atlases/Conte69_atlas.LR.32k_fs_LR_glasser/fsaverage_LR32k';
sphere = gifti([surfdir '/sub013.' hem '.sphere.32k_fs_LR.surf.gii']);%gifti([surfdir '/Conte69.' hem '.sphere.32k_fs_LR.surf.gii']);
...
%         clusters = metric_cluster(rotatedparcel,.5,10,0,[]);
%         sizes = sum(single(logical(clusters)),1);
%         [maxsize maxi] = max(sizes);
%         rotparcelindices = find(clusters(:,maxi));
%         
%         if ((maxsize / sum(sizes)) < .75)
%             
%             rotparcelindices = find(geo_distances(rotcenterind,:) < maxdist);
%             
%         end
            
        
        if (numel(intersect(rotparcelindices,find(mask))) == 0) && (numel(intersect(rotparcelindices,find(~gooddata))) <= minsize)
        ...
        if (numel(intersect(rotparcelindices,find(mask))) == 0)  && ((numel(borderverts) / numel(rotparcelindices)) < 3) && (numel(intersect(rotparcelindices,find(~gooddata))) <= minsize)
