...
Datasavename = 'PairwiseData_constrained_withintask_slidingwindow_equalrunlengths';

analysis_dir = '/fmri/data3/Evan/Gene-Rest-Nback/Analysis/ICA/ICA_79.gica/ROIs/Network/';
...
behavname = {'RT','RT3B','Acc','Acc100%','Acc3B','ICV','ICV_3Back'};
...
%roi_names = {'rFPC-rdlPFC'    'rFPC-rvlPFC'    'rFPC-rPar'    'lFPC-ldlPFC'    'lFPC-lPar'   'Sal-raIns'  'Sal-laIns'    'Sal-raMFG'   'Sal-laMFG'    'aDMN-vmPFC'  'aDMN-PCC'   'aDMN-lAG' 'pDMN-rPrec'   'pDMN-lPrec'   'pDMN-lAG'};
...
%                 RunTtestVal(i,j) = stats.tstat;
            ...
            for run = 1:length(runs)
                ...
                [h,p,ci,stats] = ttest2(reshape(Stdwindowcorrelvectors(i,j,find(DATgenotype==2),run),[1,length(find(DATgenotype==2))]),reshape(Stdwindowcorrelvectors(i,j,find(DATgenotype==1),run),[1,length(find(DATgenotype==1))]),basealpha);
                DATTtestTVal{run}(i,j) = stats.tstat;
                DATTtestPVal{run}(i,j) = p;
...
%                             runperm = randperm(2);
%                             for run = 1:length(runs)
%                                 designmatrix = [designmatrix ; [Stdwindowcorrelvectors(i,j,subj,run) DATgenotype(subjperm(subj)) runperm(run) subjperm(subj)]];
...
%             RunTtestVal(i,j) = 0;
            DATTtestTVal{1}(i,j) = 0; DATTtestTVal{2}(i,j) = 0;
            DATTtestPVal{1}(i,j) = 1; DATTtestPVal{2}(i,j) = 1;
...
for run=1:length(runs)
    ...
    SigTtests = DATTtestPVal{run} < thisalpha;
    SigTtestTVal = DATTtestTVal{run} .* SigTtests;
    
    imagesc(SigTtestTVal,[Tlimit,-Tlimit])
...
if behaviorcorrel ==1
    ...
    for behavnum = 1:length(behavior)
        ...
        if useFDR; SigCorrelVal{run} = CorrelVal{run} .* CorrelFDRBin{run}; else; SigCorrelVal{run} = CorrelVal{run} .* (CorrelSig{run} < alpha) ; end
...
if makenew ==1
    save([analysis_dir Datasavename],'Stdwindowcorrelvectors','ANOVA_FVal_DAT','ANOVA_FVal_Model','ANOVA_FVal_Run','ANOVA_FVal_RunxDAT','ANOVA_PVal_DAT','ANOVA_PVal_Model','ANOVA_PVal_Run','ANOVA_PVal_RunxDAT','FDRpvectorDAT','FDRpvectorModel','FDRpvectorRun','FDRpvectorRunxDAT','RunTtestVal','DATTtestVal')
