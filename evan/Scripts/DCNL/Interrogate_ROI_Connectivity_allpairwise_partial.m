...
outputfilename = 'ConnectivityOutput_filt_partial.txt';
%'ConnectivityOutput_spm8.txt';

Datasavename = 'PairwiseData_filt_partial';
            ...
            for i = 1:length(roi_names)
                for j = 1:length(roi_names)
                    ...
                    if i>j
                        ...
                        if strcmp(runs{run},'Nback')
                            Correlvals(i,j) = partialcorr(Timecourses{run}(:,i),Timecourses{run}(:,j),[LV_timecourse{run} WM_timecourse{run} motionparams taskregressor Timecourses{run}(:,1:j-1) Timecourses{run}(:,j+1:i-1) Timecourses{run}(:,i+1:end)]);
                        else
                            Correlvals(i,j) = partialcorr(Timecourses{run}(:,i),Timecourses{run}(:,j),[LV_timecourse{run} WM_timecourse{run} motionparams Timecourses{run}(:,1:j-1) Timecourses{run}(:,j+1:i-1) Timecourses{run}(:,i+1:end)]);
                        end
                        
                        %Correlvals(i,j) = partialcorr(Timecourses{run}(:,i),Timecourses{run}(:,j),[Timecourses{run}(:,1:j-1) Timecourses{run}(:,j+1:i-1) Timecourses{run}(:,i+1:end)]);
...
for i = 1:length(roi_names)
    for j = 1:length(roi_names)
        ...
        if i>j
            ...
            [h,p,ci,stats] = ttest2((Fishermatrix(i,j,find(DATgenotype==2),2)-Fishermatrix(i,j,find(DATgenotype==2),1)),(Fishermatrix(i,j,find(DATgenotype==1),2)-Fishermatrix(i,j,find(DATgenotype==1),1)),basealpha);
            WM_min_Rest_TtestTval(i,j) = stats.tstat;
            WM_min_Rest_TtestPval(i,j) = p;
            ...
            WM_min_Rest_TtestTval(i,j) = 0;
            WM_min_Rest_TtestPval(i,j) = 0;
        end
...
%%%%% STATE EFFECT %%%%%%
%------------------------------------------------------------------------
Tlimit = -tinv(1-(basealpha/5000),length(subjects)-2);
...
% 
% 
% SigWM_min_Rest_TtestTval = WM_min_Rest_TtestTval .* (WM_min_Rest_TtestPval < thisalpha);
% 
% imagesc(SigWM_min_Rest_TtestTval,[Tlimit,-Tlimit])
% set(gca,'XTick',[1:length(roi_names)]); set(gca,'YTick',[1:length(roi_names)]);
% set(gca,'XTickLabel',roi_names);
% set(gca,'YTickLabel',roi_names);
% title(['DAT T-test: WM-Rest']);
% colormap hot; hotmap = colormap; coolmap = [hotmap(:,3),hotmap(:,2),hotmap(:,1)];
% hotmap = hotmap(1:end-3, :); coolmap = coolmap(1:end-3, :);
% combined = [flipdim(coolmap,1); zeros(10,3); hotmap];
% colormap(combined);
...
if makenew ==1
    save([analysis_dir Datasavename],'subjects','roi_names','Fishermatrix','ANOVA_FVal_DAT','ANOVA_FVal_Model','ANOVA_FVal_Run','ANOVA_FVal_RunxDAT','ANOVA_PVal_DAT','ANOVA_PVal_Model','ANOVA_PVal_Run','ANOVA_PVal_RunxDAT','FDRpvectorDAT','FDRpvectorModel','FDRpvectorRun','FDRpvectorRunxDAT','RunTtestVal','DATTtestVal')
