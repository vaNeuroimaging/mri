...
for subject = 1:length(subjects)
    ...
    datafiles3D = [data_location 'swa*.hdr'];
...
%     evalc(['!fslmaths /fmri/data3/Evan/Gene-Rest-Nback/Analysis/TBSS/BDNF_77/stats/mean_FA_skeleton_mask_inv_thr4_4mm.nii.gz -thr .5 -bin /fmri/data3/Evan/Gene-Rest-Nback/Analysis/TBSS/BDNF_77/stats/mean_FA_skeleton_mask_inv_thr4_4mm.nii.gz']);
    ...
    evalc(['!/data/apps/fsl/4.1.7/bin/probtrackx --mode=seedmask -x ' seedimagename ' -l -c 0.05 -S 2000 --steplength=0.5 -P 5000 --xfm=' DTIdir 'bothruns.bedpostX/xfms/4mm2diff.mat --forcedir --opd -s ' DTIdir 'bothruns.bedpostX/merged -m ' DTIdir '/bothruns.bedpostX/nodif_brain_mask  --dir=' outputfile 'temp']);
    ...
    for voxel = 1:length(pathpresentindices);
        ...
        evalc(['!/data/apps/fsl/4.1.7/bin/probtrackx --network --mode=seedmask -x ' outputfile 'temp/temp_target.nii --waypoints=' seedimagename ' --stop=' seedimagename ' -l -c 0.05 -S 1000 --steplength=1 -P 500 --xfm=' DTIdir 'bothruns.bedpostX/xfms/4mm2diff.mat --forcedir --opd -s ' DTIdir 'bothruns.bedpostX/merged -m ' DTIdir 'bothruns.bedpostX/nodif_brain_mask  --dir=' outputfile 'voxeltemp']);
