...
for seednum = 1:length(seeds)
    ...
    for subject = 1:length(subjects)
        ...
        for cond = 1:length(conditions)
            ...
            outputimgname = [shortseedname '_' runs{run} '_' conditions{cond} '.nii'];
            ...
            if strcmp(runs{run},'Nback')
                P = P(timepointstouse{cond},:);
                motionparams = motionparams(timepointstouse{cond},:);
            else
                P = P(1:108,:);
            ...
            while getdataworked == 0
                try
                    ...
                    [seed_voxels a b c] = getdata(seed{1}, P,'l');
...
%             residual_seed_timecourse = mean(residuals,2);
            ...
            if strcmp(runs{run},'Nback')
                datamatrix = datamatrix(:,:,:,timepointstouse{cond});
            else
...
%                 if strcmp(runs{run},'Nback')
%                     R = jacket_partialcorr(seed_timecourse, reshapeddata, [CSF_timecourse WM_timecourse motionparams taskregressor]);
%                 else
            ...
            outputimage = make_nii(output,[2 2 2],data.hdr.hist.originator(1:3));
