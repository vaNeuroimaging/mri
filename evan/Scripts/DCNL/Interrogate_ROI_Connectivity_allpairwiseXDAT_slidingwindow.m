...
Datasavename = 'PairwiseData_constrained_withintask_slidingwindow_nonequalrunlengths';

analysis_dir = '/fmri/data3/Evan/Gene-Rest-Nback/Analysis/ICA/ICA_79.gica/ROIs/Network/TPN-TNN/';
...
%                 motionparams = motionparams(1:108,:);
            ...
            for roinum = 1:length(roi_names)
                ...
                Timecourses{run}(:,roinum,subject) = residual_timepoint_means;
                
            end
            
            
            for windownum = 1:(length(Timecourses{run}(:,1,subject))-windowsize)
                correlvals = corrcoef(Timecourses{run}(windownum:windownum+windowsize,:,subject));
    ...
    for j = 1:length(roi_names)
        ...
        if i>j
            ...
            %if makenew==1
                ...
                [h,p,ci,stats] = ttest2(reshape(Stdwindowcorrelvectors(i,j,find(DATgenotype==2),:),[1,length(find(DATgenotype==2))*2]),reshape(Stdwindowcorrelvectors(i,j,find(DATgenotype==1),:),[1,length(find(DATgenotype==1))*2]),basealpha);
                ...
                for subj = 1:length(subjects)
                    for run = 1:length(runs)
                        designmatrix = [designmatrix ; [Stdwindowcorrelvectors(i,j,subj,run) DATgenotype(subj) run subj]];
                    end
                end
...
%Testing effects seperately
%------------------------------------------------------------------------
%%%%% STATE EFFECT %%%%%%
%------------------------------------------------------------------------
% if useFDR == 1;
%
%     thisalpha = FDR(FDRpvectorRun, basealpha);
%     if isempty(thisalpha); thisalpha = 0; end
%
% elseif useBonf == 1;
%
...
% %Unthresholded
%
% imagesc(RunTtestVal,[Tlimit,-Tlimit])
% set(gca,'XTick',[1:length(roi_names)]); set(gca,'YTick',[1:length(roi_names)]);
% set(gca,'XTickLabel',roi_names);
% set(gca,'YTickLabel',roi_names);
% title([runs{2} ' vs ' runs{1} ': Unthresholded T Values']);
% colormap hot; hotmap = colormap; coolmap = [hotmap(:,3),hotmap(:,2),hotmap(:,1)];
% hotmap = hotmap(1:end-3, :); coolmap = coolmap(1:end-3, :);
% combined = [flipdim(coolmap,1); zeros(10,3); hotmap];
% colormap(combined);
...
% if useFDR == 1;
%
%     thisalpha = FDR(FDRpvectorDAT, basealpha);
%     if isempty(thisalpha); thisalpha = 0; end
%
% elseif useBonf == 1;
%
...
%
% elseif useMonteCarlo == 1;
%     pvector_MonteCarlo_DAT = sort(pvector_MonteCarlo_DAT);
%     thisalpha = pvector_MonteCarlo_DAT(comparisoncounter*simulations*basealpha)/comparisoncounter
%
% else
%
%     thisalpha = basealpha;
...
%
%
% SigANOVA_PVal_DAT = ANOVA_PVal_DAT < thisalpha;
%
% DATSigTtestVal = DATTtestVal .* SigANOVA_PVal_DAT;
%
% imagesc(DATSigTtestVal,[Tlimit,-Tlimit])
...
% if useFDR == 1;
%
%     thisalpha = FDR(FDRpvectorRunxDAT, basealpha);
%     if isempty(thisalpha); thisalpha = 0; end
%
% elseif useBonf == 1;
%
...
%
% elseif useMonteCarlo == 1;
%     pvector_MonteCarlo_RunxDAT = sort(pvector_MonteCarlo_RunxDAT);
%     thisalpha = pvector_MonteCarlo_RunxDAT(comparisoncounter*simulations*basealpha)/comparisoncounter
%
% else
%
%     thisalpha = basealpha;
...
%
%
% SigANOVA_PVal_RunxDAT = ANOVA_PVal_RunxDAT < thisalpha;
%
% RunxDATSigFtestVal = ANOVA_FVal_RunxDAT .* SigANOVA_PVal_RunxDAT;
%
% imagesc(RunxDATSigFtestVal,[0,Flimit])
...
%
%
%     TtestBin{run} = zeros(length(roi_names));
%     TtestSig{run} = zeros(length(roi_names));
%     TtestVal{run} = zeros(length(roi_names));
%     TtestFDRBin{run} = zeros(length(roi_names));
%
%
%     MeanFisher{run} = mean(Stdwindowcorrelvectors(:,:,:,run),3);
%     MeanFisher_910{run} = mean(Stdwindowcorrelvectors(:,:,find(DATgenotype==1),run),3);
%     MeanFisher_1010{run} = mean(Stdwindowcorrelvectors(:,:,find(DATgenotype==2),run),3);
%
%     pvector = [];
%
%     comparisoncounter = 0;
...
%             if i>j
%                 comparisoncounter = comparisoncounter + 1;
%                 [h,p,ci,stats] = ttest2(Stdwindowcorrelvectors(i,j,find(DATgenotype==2),run),Stdwindowcorrelvectors(i,j,find(DATgenotype==1),run),alpha);
%                 TtestBin{run}(i,j) = h;
%                 TtestSig{run}(i,j) = p;
%
%                 pvector(end+1) = p;
...
%
%             elseif i == j
%                 MeanFisher{run}(i,j) = Inf;
%                 MeanFisher_910{run}(i,j) = Inf;
%                 MeanFisher_1010{run}(i,j) = Inf;
%             else
%                 MeanFisher{run}(i,j) = 0;
%                 MeanFisher_910{run}(i,j) = 0;
%                 MeanFisher_1010{run}(i,j) = 0;
%             end
%
%         end
%     end
...
for run = 1:length(runs)
    ...
    imagesc(MeanFisher_910{run}.*valuestokeep,[0,.15])
    set(gca,'XTick',[1:length(roi_names)]); set(gca,'YTick',[1:length(roi_names)]);
    set(gca,'XTickLabel',roi_names);
    set(gca,'YTickLabel',roi_names);
    ...
    imagesc(MeanFisher_1010{run}.*valuestokeep,[0,.15])
    set(gca,'XTick',[1:length(roi_names)]); set(gca,'YTick',[1:length(roi_names)]);
    set(gca,'XTickLabel',roi_names);
    set(gca,'YTickLabel',roi_names);
...
imagesc(MeanFisher_all.*valuestokeep,[0,.15])
set(gca,'XTick',[1:length(roi_names)]); set(gca,'YTick',[1:length(roi_names)]);
set(gca,'XTickLabel',roi_names);
set(gca,'YTickLabel',roi_names);
...
if makenew ==1
    save([analysis_dir Datasavename],'Stdwindowcorrelvectors','Fisherwindowcorrelvectors','ANOVA_FVal_DAT','ANOVA_FVal_Model','ANOVA_FVal_Run','ANOVA_FVal_RunxDAT','ANOVA_PVal_DAT','ANOVA_PVal_Model','ANOVA_PVal_Run','ANOVA_PVal_RunxDAT','FDRpvectorDAT','FDRpvectorModel','FDRpvectorRun','FDRpvectorRunxDAT','RunTtestVal','DATTtestVal')
