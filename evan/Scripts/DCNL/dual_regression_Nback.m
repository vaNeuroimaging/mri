...
for subnum = 1:length(subjects)
    ...
    %--------------------------------------------------------------------------
    Rest_datafolder = ['/fmri/data3/Evan/Gene-Rest-Nback/Data/' subj '/SPM8/Nback/Temp/'];
    Outputfolder = ['/fmri/data3/Evan/Gene-Rest-Nback/Analysis/' subj '/SPM8/DualRegression_Nback_ICA76/'];
    %----------------------------------------------------------------------

    try rmdir(Rest_datafolder,'s'); catch; end
    mkdir([Rest_datafolder]);
    FSL_from_matlab(['fslsplit ' Rest_datafolder '../Nbackonly_smoothed.nii.gz ' Rest_datafolder prefix ' -t']);
    ...
    for time = 1:num_timepoints
        ...
        string{time} = ['Spatial regression: Subject ' subj ', timepoint ' num2str(time)];
