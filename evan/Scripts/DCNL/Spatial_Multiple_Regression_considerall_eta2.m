...
outputfilename = ['/fmri/data3/Evan/Gene-Rest-Nback/Analysis/Task_made_of_components/Task_made_of_components_eta2.txt'];
...
fprintf(fid,'%s\t\%s\t\%s\t\%s\t\%s\n\r\','Subject','Condition','GroupComp','IndComp','eta-sq','Beta','R-sq');
fclose(fid);
...
%     end
    ...
    for i = 1:numbergroupcomponents
        for j = 1:size(valid_component_index)
            Correlvals(i,j) = eta2(Groupcomponentdata(:,i),TestingComponentdata(:,j));
        end
    end
