...
amyg_vmPFC_fc = zeros(length(subjects),1);
...
for s = 1:length(subjects)
    ...
    amygtc = mean(data.data((structinds==5) | (structinds==6) , :),1)';
    vmPFCtc = mean(data.data(vmPFCinds,:),1)';
    amyg_vmPFC_fc(s) = FisherTransform(paircorr_mod(amygtc,vmPFCtc));
end
...
[P,ANOVATAB,STATS] = anova1(amyg_vmPFC_fc,tbivec);
ANOVATAB
 [H,P,CI,STATS] = ttest2([amyg_vmPFC_fc(tbivec==1) ; amyg_vmPFC_fc(tbivec==2)],amyg_vmPFC_fc(tbivec==0));
 STATS
 P


figure('Color','white','position',[1982 478 1352 804]);
bh = boxplot(amyg_vmPFC_fc,tbivec,'colors','k','symbol','k','labels',{'No TBI','Mild TBI','Moderate TBI'});
set(bh,'linewidth',3)
...
[r_within,p_within] = corr(tbinumvec(test_subinds),amyg_vmPFC_fc(test_subinds))%,'type','Spearman')
prettyscatterplot(tbinumvec(test_subinds),amyg_vmPFC_fc(test_subinds),1)
%export_fig(gca,'TBInum_vs_WithinRSFC.pdf')



%[r,p] = partialcorr(tbinumvec,amyg_vmPFC_fc,tbivec)
...
[r_within,p_within] = corr(PCLvec(PCL_subinds),amyg_vmPFC_fc(PCL_subinds))%,'type','Spearman')
prettyscatterplot(amyg_vmPFC_fc(PCL_subinds),PCLvec(PCL_subinds),1)
...
[~,T,~,~]=anovan(PCLvec(PCL_subinds)',{tbivec(PCL_subinds),amyg_vmPFC_fc(PCL_subinds)},'continuous',2)
...
[r_within,p_within] = corr(uncinateFA,amyg_vmPFC_fc)%,'type','Spearman')
figure;plot(uncinateFA,amyg_vmPFC_fc,'k.','MarkerSize',20)
...
[~,T,~,~]=anovan(PCLvec(PCL_subinds)',{uncinateFA(PCL_subinds),amyg_vmPFC_fc(PCL_subinds)},'continuous',[1 2])
% 
...
[~,T,~,~]=anovan(PCLvec(PCL_subinds)',{tbivec(PCL_subinds),uncinateFA(PCL_subinds),amyg_vmPFC_fc(PCL_subinds)},'continuous',[2 3])


%% FA/FC vs PCL control for TBI number and severity

[r,p] = partialcorr([PCLvec(PCL_subinds) uncinateFA(PCL_subinds) amyg_vmPFC_fc(PCL_subinds) tbivec(PCL_subinds) tbinumvec(PCL_subinds)])
